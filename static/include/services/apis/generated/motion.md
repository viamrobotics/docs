...
- `slam_service_name` ([str](https://docs.python.org/3/library/stdtypes.html#text-sequence-type-str)) (required): The `ResourceName` of the SLAM service from which the SLAM map is requested.
...
- `obstacle_detectors` [(Iterable[ObstacleDetector])](https://python.viam.dev/autoapi/viam/proto/service/motion/index.html#viam.proto.service.motion.ObstacleDetector): The names of each [vision service](/reference/services/vision/) and [camera](/reference/components/camera/) resource pair you want to use for transient obstacle avoidance.
...
- `SlamName` [(resource.Name)](https://pkg.go.dev/go.viam.com/rdk/resource#Name): The `resource.Name` of the SLAM service from which the SLAM map is requested.
- `MotionConfig` [(\*MotionConfiguration)](https://pkg.go.dev/go.viam.com/rdk/services/motion#MotionConfiguration): The configuration you want to set across this machine for this motion service. This parameter and each of its fields are optional.
  - `ObstacleDetectors` [([]ObstacleDetectorName)](https://pkg.go.dev/go.viam.com/rdk/services/motion#ObstacleDetectorName): The names of each [vision service](/reference/services/vision/) and [camera](/reference/components/camera/) resource pair you want to use for transient obstacle avoidance.
...
Move a [base](/reference/components/base/) component to a destination GPS point, represented in geographic notation _(latitude, longitude)_.
Use a [movement sensor](/reference/components/movement-sensor/) to check the location of the machine.
...
`MoveOnGlobe()` is intended for use with the [navigation service](/reference/services/navigation/), providing autonomous GPS navigation for rover [bases](/reference/components/base/).

{{< alert title="Requirements" color="info" >}}
To use `MoveOnGlobe()`, your movement sensor must be able to measure the GPS location and orientation of the machine.

Make sure the [movement sensor](/reference/components/movement-sensor/) you use supports usage of the following methods in its {{< glossary_tooltip term_id="model" text="model's" >}} implementation of the [movement sensor API](/reference/apis/components/movement-sensor/).
...
- `movementSensorName` [(resource.Name)](https://pkg.go.dev/go.viam.com/rdk/resource#Name): The `resource.Name` of the [movement sensor](/reference/components/movement-sensor/) that you want to use to check the machine's location.
...
The following code example gets the pose of the tip of a [gripper](/reference/components/gripper/) named `my_gripper` which is attached to the end of an arm, in the "world" `reference_frame`:
...
By default, returns the plan history of the most recent [`MoveOnGlobe`](/reference/apis/services/motion/#moveonglobe) or [`MoveOnMap`](/reference/apis/services/motion/#moveonmap) call to move a [base](/reference/components/base/) component.
