...
| ---- | ----------- | ----------------------------------- |
| `VIAM_HOME` | The root of the `viam-server` configuration. You can set the `VIAM_HOME` environment variable before starting `viam-server` to customize this directory location.<br>Default for Linux, macOS, and WSL: <file>$HOME/.viam</file><br>Default for Windows: <file>C:\WINDOWS\system32\config\systemprofile\\.viam</file><br>On Windows, if you are running from a drive other than C:, you must set `VIAM_HOME` to a path on the same drive (for example, <file>D:\\.viam</file>) for modules to function correctly. | <p class="center-text"><i class="fas fa-check" title="yes"></i></p> |
