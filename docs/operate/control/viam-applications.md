...
| `models` | array | Optional | Deprecated. Empty unless you are shipping the app alongside models. For information on how to add models, see [Support hardware and software](/operate/modules/write-a-driver-module/). |
...
| ------------ | ------ | ----------- |
| `name`       | string | The name of your application, which will be a part of the application's URL (`name_publicnamespace.viamapplications.com`). For more information on valid names see [Valid application identifiers](/operate/modules/advanced/metajson/#valid-application-identifiers). |
