...
| `block_downloads_on_low_disk`       | boolean | `false` | Refuse agent self-updates and downloads of the viam-server binary when a download would leave less than 10MB free on the cache volume. When `false`, the agent logs a warning and downloads anyway. To also block module and package downloads, see [Block downloads on low disk space](/reference/viam-agent/#block-downloads-on-low-disk-space). |
