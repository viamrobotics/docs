...
| `schedule` | string | **Required** | Schedule specification using unix-cron format or Golang duration. Values must be non-zero. Accepts <ul><li>Unix-cron expressions for time-based scheduling:<ul><li>`"*/5 * * * * *"`: Every 5 seconds (6-field cron with seconds)</li><li>`"0 */6 * * *"`: Every 6 hours</li><li>`"0 0 * * 0"`: Every Sunday at midnight</li><li>`"*/15 * * * *"`: Every 15 minutes</li><li>`"0 9 * * 1-5"`: Every weekday at 9 AM</li></ul></li><li>Golang duration strings for interval-based scheduling:<ul><li>`"10ms"`: Every 10 milliseconds</li><li>`"30s"`: Every 30 seconds</li><li>`"5m"`: Every 5 minutes</li><li>`"1h"`: Every hour</li><li>`"24h"`: Every 24 hours</li></ul></li></ul>Job schedules are evaluated in the machine's local timezone. |
...
| `log_configuration` | object | Optional | Override log level for this job. Set `"level"` to `"debug"`, `"info"`, `"warn"`, or `"error"`. |
...
8/19/2025, 7:38:59 PM debug rdk.job_manager.periodic-job   jobmanager/jobmanager.go:160   Job succeeded   response map[action:true]
8/19/2025, 7:38:59 PM debug rdk.job_manager.periodic-job   jobmanager/jobmanager.go:155   Job triggered
...
To see "Job triggered" and "Job succeeded" messages, set the log level to debug for viam-server or use the `log_configuration` field on the job.
