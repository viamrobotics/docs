...
configuration change in plain language and have the **AI assistant** propose it
for you, instead of adding each resource through the form fields or writing the
JSON by hand. You type a request such as "Add a fake sensor named test-sensor",
and the assistant replies in a panel and stages the change for you to review.
The change takes effect on your machine only when you click **Save**.
...
- **Where it lives.** The assistant is available on any tab of your machine's
  page and on the [fragment](/hardware/fragments/) configuration page. Open it
  from the **AI assistant** button (sparkle icon) at the bottom right. The
...
1. Open your machine's page in the [Viam app](https://app.viam.com).
2. Click the **AI assistant** button (sparkle icon) at the bottom right. The
...
When the panel is empty, it shows suggested starter requests you can select
