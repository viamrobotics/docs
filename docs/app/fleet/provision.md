...
Such a machine might use Viam to interface between a [sensor component](/machine/components/sensor/) that takes weather measurements, and the [data management service](/app/data/) to regularly upload a stream of readings, for example.
...
The example video shows using the [Viam mobile application](/app/fleet/#the-viam-mobile-app) to connect to the Viam Agent on a newly-deployed machine and completing network setup.
...
1. Determine your machine {{< glossary_tooltip term_id="part" text="part's" >}} ID.
   ...
   For an example, see [Find part ID](/program/apis/data-client/#find-part-id)
1. Determine your machine's [API key and API key ID](/program/#authenticate).
...
1. Create a configuration file with the desired configuration for your machine. You can:
   ...
   - You can [create a new machine in the Viam app](/app/fleet/machines/#add-a-new-machine) and configure it as desired, then switch to **Raw JSON** mode and copy the configuration shown into a new file on your machine.
...
This file configures some basic metadata, specifies a [fragment](/app/fleet/configure-a-fleet/) to use to configure the machine, and provides the WiFi network name and password to allow your machine to connect automatically on startup.
...
You can use the [Viam mobile application](/app/fleet/#the-viam-mobile-app), available for download from the [Apple](https://apps.apple.com/us/app/viam-robotics/id6451424162) and [Google Play](https://play.google.com/store/apps/details?id=com.viam.viammobile&hl=en&gl=US) app stores, to connect to the Viam Agent on deployed machines.
