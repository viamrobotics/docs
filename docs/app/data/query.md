...
icon: true
images: ["/machine/services/icons/data-query.svg"]
aliases:
  - /manage/app/data/query/
# SME: Devin Hilly
---

Once you have [added the data management service](/app/data/capture/#add-the-data-management-service) and [synced tabular data to the Viam app](/app/data/cloud-sync/), you can perform queries against that data using either {{< glossary_tooltip term_id="sql" text="SQL" >}} or {{< glossary_tooltip term_id="mql" text="MQL" >}}.
...
You can run queries against both the captured tabular data itself as well as its metadata, including machine ID, organization ID, and [tags](/app/data/dataset/#image-tags).

Only tabular data, such as [sensor](/machine/components/sensor/) readings, can be queried using SQL or MQL.
To search non-tabular data, such as images, see [Filter Data](/app/data/view/#filter-data).
...
1. [Add the data management service](/app/data/capture/#add-the-data-management-service) to your machine.
1. [Configure data capture](/app/data/capture/) for at least one component, such as a sensor.
   Only components that capture tabular data support data query.
   To search non-tabular data, see [Filter Data](/app/data/view/#filter-data).
1. [Configure cloud sync](/app/data/cloud-sync/), and sync data to the Viam app.
   When you are able to [view your data in the Viam app](/app/data/view/), you are ready to proceed.

## Query tabular data in the Viam app

Once you have synced tabular data to the Viam app, you can run SQL or MQL queries against your synced data from the [**Query** subtab](https://app.viam.com/app/data/query) under the **Data** tab in the Viam app.
You must have the [owner](/app/fleet/rbac/) role in order to query data in the Viam app.

1. Navigate to the [**Query** subtab](https://app.viam.com/app/data/query).
...
1. Select either `SQL` or `MQL` from the **Query mode** dropdown menu on the right-hand side, then enter your query using the respective syntax for the language you have selected in the text area at the top of your screen.
   ...
   - The following shows a SQL query that filters by the component name `my-ultrasonic-sensor` and limits the returned results to 5:
     ...
     {{< imgproc src="/app/data/query-ui-sql.png" alt="Viam App Data Query tab with a SQL query shown" resize="800x" >}}

   - The following shows the same search using MQL syntax:

     {{< imgproc src="/app/data/query-ui-mql.png" alt="Viam App Data Query tab with an MQL query shown" resize="800x" >}}
...
1. Click **Run query** when ready to perform your query and get matching results.
   ...
   - The following shows a SQL query that filters by component name and specific column names, and its returned results:
     ...
     {{< imgproc src="/app/data/query-ui-results.png" alt="Viam App Data Query tab with a SQL query shown and results shown below including two matching records" resize="800x" >}}

   - The following shows a SQL query that returns a count of records matching the search criteria:

     {{< imgproc src="/app/data/query-ui-numreadings.png" alt="Viam App Data Query tab with a SQL query shown with the resulting count of matching records displayed below" resize="800x" >}}
...
You can query against both the captured tabular data itself as well as its metadata, including machine ID, organization ID, and [tags](/app/data/dataset/#image-tags).

Only tabular data, such as [sensor](/machine/components/sensor/) readings, can be queried in this fashion.
...
You do not need to perform any additional configuration when [querying data in the Viam app](/app/data/query/#query-tabular-data-in-the-viam-app).
{{< /alert >}}
...
1. Open your chosen MQL-compatible client an connect to the Viam organization's MongoDB Atlas instance.
   ...
   To connect, use the `hostname` you determined when you [configured direct data query](/app/data/query/#configure-data-query), and structure your username in the following format:
...
1. Run the following command to connect to the Viam organization's MongoDB Atlas instance from `mongosh`:
   ...
   - `<YOUR-DB-HOSTNAME>` is your organization's assigned MongoDB Atlas instance hostname, determined from the [`viam data database hostname` CLI command](/app/data/query/#configure-data-query).
...
See [Visualize Data](/app/data/visualize/) for instructions on setting up and using these data visualization services with Viam, or the [Visualize data with Grafana](/tutorials/services/visualize-data-grafana/) tutorial for a detailed walkthrough specific to Grafana.

To export your captured data from the cloud, see [Export Data](../export/).

To adjust the rate at which your machine captures data, see [Configure Data Capture](/app/data/capture/#configure-data-capture-for-individual-components).

To adjust the sync frequency, see [Configure Cloud Sync](/app/data/cloud-sync/).
