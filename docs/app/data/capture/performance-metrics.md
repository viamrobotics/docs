...
icon: true
images: ["/app/data/monitor.gif"]
videos: ["/app/data/monitor.webm", "/app/data/monitor.mp4"]
aliases:
...
To capture data from a machine, you need to [create a machine in Viam](/app/fleet/machines/#add-a-new-machine) and follow the setup instructions to install `viam-server` and connect it to the Viam app.
...
To obtain performance metrics about your machine, use the [`viam-telegraf-sensor`](https://app.viam.com/module/viam/viam-telegraf-sensor) module from the [Viam registry](/app/registry/).
The module provides a [sensor](/machine/components/sensor/) that allows you to obtain readings containing your machine's performance metrics.
...
For more information, see [add a module from the Viam Registry](/app/registry/configure/#add-a-modular-resource-from-the-viam-registry).
...
{{<gif webm_src="/app/data/monitor.webm" mp4_src="/app/data/monitor.mp4" alt="sensor control tab">}}

## Configure data management service to capture and sync sensor data

To capture the data from your configured sensor, you need to add the [data management service](/app/data/) and configure it to capture and sync the sensor data:
...
View your sensor data on the [**DATA** tab](https://app.viam.com/app/data/view?view=sensors).

![View of sensor data](/app/data/sensor-data.png)
