...
The code for both of these examples is provided by the [trigger-sync-examples module](https://github.com/viam-labs/trigger-sync-examples-v2), and you can [create your own module](/app/registry/create/) if you want to use different logic.

{{% alert title="Note: How sync is triggered" color="note" %}}

Regardless of the specifics of your trigger sync logic, to trigger sync you need to pass `true` to the [CreateShouldSyncReading function](https://pkg.go.dev/go.viam.com/rdk/machine/services/datamanager#CreateShouldSyncReading) within the definition of your modular sensor's `Readings` function.
...
Before configuring your sensor, you must [create a machine](/app/fleet/machines/#add-a-new-machine) and you also need to:

1. Enable [data capture](/app/data/capture/).
2. Enable [cloud sync](/app/data/cloud-sync/).
...
To test your setup, [configure a webcam](/machine/components/camera/webcam/) or another component and [enable data capture on the component](/app/data/capture/#configure-data-capture-for-individual-components).
For a camera component, use the `ReadImage` method.
The data manager will now capture data.
Go to the [**CONTROL** tab](/app/fleet/control/).
You should see the sensor.
Click on `GetReadings`.

{{<imgproc src="/app/data/timesensor.png" resize="800x" declaredimensions=true alt="Control tab with sensor panel">}}

If you are in the time frame for sync, the time sync sensor will return true.
You can confirm that no data is currently syncing by going to the [**Data** tab](https://app.viam.com/app/data/view).
