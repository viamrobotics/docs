...
no_list: true
icon: true
images: ["/app/registry/upload-module.svg"]
aliases:
  - "/extend/modular-resources/upload/"
  - "/modular-resources/upload/"
---

Once you have [created a custom module](/app/registry/create/), use the instructions on this page to upload it to the Viam registry as a public module that is shared with other Viam users, or as a private module that is shared only within your [organization](/app/fleet/organizations/).
...
2. Next, run the `viam module create` command to choose a custom module name and generate the required metadata for your module.
   By default, a new module is created as _private_, meaning that it is only accessible to members of your [organization](/app/fleet/organizations/), but you can choose to set the `visibility` of your module to _public_ to make it accessible to all Viam users.
...
1.  If you haven't already, [create a new namespace](/app/fleet/organizations/#create-a-namespace-for-your-organization) for your organization.
...
3.  Edit the newly-created `meta.json` file, and provide the required configuration information for your custom module by filling in the following fields.
    ...
    <table class="table table-striped">
      ...
      <tr>
        ...
        <td>The module ID, which includes either the module <a href="/app/fleet/organizations/#create-a-namespace-for-your-organization">namespace</a> or <a href="/app/fleet/organizations/">organization-id</a>, followed by its name (pre-populated using the <code>--name</code> you provided in the <code>viam module create</code> command).
      ...
      <tr>
        ...
        <td>object</td>
        <td><strong>Required</strong></td>
        <td><p>A list of one or more {{< glossary_tooltip term_id="model" text="models" >}} provided by your custom module. You must provide at least one model, which consists of an <code>api</code> and <code>model</code> key pair. If you are publishing a public module (<code>"visibility": "public"</code>), the namespace of your model must match the <a href="/app/fleet/organizations/#create-a-namespace-for-your-organization">namespace of your organization</a>.</p><p>For more information, see <a href="/app/registry/#naming-your-model-namespacerepo-namename">naming your model</a>.</p></td>
      </tr>
      <tr>
...
For more information, see [Naming your model](/app/registry/#naming-your-model-namespacerepo-namename).
...
Where `dist/main` is the [packaged executable](/app/registry/create/#compile-or-package-your-module) that runs the module at the [entry point](/app/registry/create/#write-an-entry-point-main-program-file).
...
1. For modules written in Python, you should package your module files as an archive first, before uploading.
   ...
   Where `dist/main` is your [packaged executable](/app/registry/create/#compile-or-package-your-module).
...
The `build-action` GitHub action relies on a build command that you need to specify in the <file>meta.json</file> file that you created for your module when you first [uploaded it](/app/registry/upload/#upload-a-custom-module-using-the-cli).
...
1. Create an [organization API key](/cli/#create-an-organization-api-key) with the [owner](/app/fleet/rbac/#permissions) role, which the GitHub action will use to authenticate to the Viam platform, using one of the following methods:
...
- you are an [owner](/app/fleet/rbac/) in the {{< glossary_tooltip term_id="organization" text="organization" >}} that owns the module, AND
...
{{<imgproc src="/app/registry/upload/edit-module-visibility.png" resize="x1000" declaredimensions=true alt="A module page with a Visibility heading on the right side. Under it, an Edit button has appeared." max-width="700px" >}}
...
{{<imgproc src="/app/registry/upload/delete-module.png" resize="x1000" declaredimensions=true alt="A module page with the ... menu open. Delete is the only option in the menu." max-width="700px" >}}
...
{{< cards >}}
{{% card link="/app/registry/configure/" %}}
{{% card link="/app/registry/examples/" %}}
{{< /cards >}}
