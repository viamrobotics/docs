...
1. Navigate to [the Viam app](https://app.viam.com) in your browser and [add a new machine](/app/fleet/machines/#add-a-new-machine).
...
- Write a client similar to `example_audio_classification_client` that [trains its own model](/app/ml/train-model/) on existing or incoming data, as long as that model fulfils the TFLite model constraints.
  For example, you might add a [movement sensor](/machine/components/movement-sensor/) component to your machine that captures sensor readings to the built-in [data management service](/app/data/).
