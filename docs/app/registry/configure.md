...
You can extend Viam by adding a module on your machine that provides one or more {{< glossary_tooltip term_id="modular-resource" text="modular resources" >}} ([components](/machine/components/) or [services](/machine/services/)):
...
A module provides one or more {{< glossary_tooltip term_id="resource" text="modular resources" >}} (either a [component](/machine/components/) or [service](/machine/services/)).
...
To add a modular [component](/machine/components/) from the Viam registry to your machine:
...
3. Browse the list of available component types, and select the specific modular component you'd like to add.
   ...
   {{<imgproc src="/app/registry/configure/add-component-by-category.png" resize="500x" style="max-width: 400px" declaredimensions=true alt="The add a component modal showing results for the intel realsense module when searching by the category 'camera'">}}

   You can also start typing to search for a module by name or to narrow down your search results.

   {{<imgproc src="/app/registry/configure/add-component-by-name.png" resize="500x" style="max-width: 400px" declaredimensions=true alt="The add a component modal showing results for the intel realsense module when searching by the name 'realsense'">}}

4. After selecting the modular component, enter a name or use the suggested name for your modular component and click **Create** to add it to your machine's component configuration.

   {{<imgproc src="/app/registry/configure/add-component-screen.png" resize="500x" style="max-width: 400px" declaredimensions=true alt="The add a component modal showing the intel realsense module pane, with the 'Add module' button shown">}}
...
Click on the **...** icon and click **Delete**.
Confirm your selection.

### Add a modular service from the Viam registry

To add a modular [service](/machine/services/) from the Viam registry to your machine:
...
3. Browse the list of available service types and select the specific modular service you'd like to add.
   ...
   {{<imgproc src="/app/registry/configure/add-service-by-category.png" resize="500x" style="max-width: 400px" declaredimensions=true alt="The add a component modal showing results for the mlmodelservice triton module when searching by the category 'ML Model'">}}

   You can also start typing to search for a module by name or to narrow down your search results.

   {{<imgproc src="/app/registry/configure/add-service-by-name.png" resize="500x" style="max-width: 400px" declaredimensions=true alt="The add a component modal showing results for the mlmodelservice triton module when searching by the name 'triton'">}}

4. After selecting the modular service, enter a name or use the suggested name for your modular service and click **Create** to add it to your machine's service configuration.

   {{<imgproc src="/app/registry/configure/add-service-screen.png" resize="500x" style="max-width: 400px" declaredimensions=true alt="The add a component modal showing the mlmodelservice triton module pane, with the 'Add module' button shown">}}
...
Click on the **...** menu and click **Delete**.
Confirm your selection.
...
You can edit the configuration of the modular resource or resources you have added from your module at the card matching the `name` you gave the resource when adding on the **CONFIGURE** tab.
...
| ---- | ---- | --------- | ----------- |
| `namespace` | string | **Required** | The namespace of the API (the first part of the {{< glossary_tooltip term_id="api-namespace-triplet" text="API namespace triplet">}}). See [Valid APIs to implement in your model](/app/registry/#valid-apis-to-implement-in-your-model) |
| `type` | string | **Required** | The {{< glossary_tooltip term_id="subtype" text="subtype">}} of the API (the third part of the {{< glossary_tooltip term_id="api-namespace-triplet" text="API namespace triplet">}}). See [Valid APIs to implement in your model](/app/registry/#valid-apis-to-implement-in-your-model). |
...
| `depends_on` | array | Optional | The `name` of components you want to confirm are available on your machine alongside your modular resource. Often a [board](/machine/components/board/). Unnecessary if you coded [implicit dependencies](/internals/rdk/#dependency-management). |
...
{{<imgproc src="/app/registry/configure/deployed-module-with-component.png" style="max-width: 400px" resize="500x" declaredimensions=true alt="The module card">}}
...
| `VIAM_MODULE_DATA` | A persistent folder location a module can use to store data across reboots and versions. This location is a good place to store [python virtual environments](/program/python-venv/).<br>Example: `$VIAM_HOME/module-data/cloud-machine-id/my-module-name/` |
...
- If you are adding your own custom module, be sure that you have followed the steps to [create your own module](/app/registry/create/) to code and compile your module and generate an executable.
- If you are using a pre-built module, make sure you have installed the module and determined the filename of [the module's executable](/app/registry/create/#compile-or-package-your-module).
...
See the instructions to [compile your module into an executable](/app/registry/create/#compile-or-package-your-module) for more information.
...
4. Enter the [module's executable path](/app/registry/create/#compile-or-package-your-module).
...
5. Then, click the **Create** button, and click **Save** in the upper right corner to save your config.
   ...
   {{<imgproc src="/app/registry/configure/add-local-module-csi-cam.png" resize="300x" declaredimensions=true alt="The add a local module pane with name 'my-csi-cam' and executable path '/usr/local/bin/viam-csi'">}}
...
7. On the **Create** menu for a **Local component** or **Local service**:
   ...
   - Select the type of modular resource provided by your module, such as a [camera](/machine/components/camera/), from the dropdown menu.
   ...
   - Enter a name for this instance of your modular resource.
     ...
     {{<imgproc src="/app/registry/configure/add-local-module-create.png" resize="250x" declaredimensions=true alt="The add a component model showing the create a module step for an intel realsense module">}}
...
{{<imgproc src="/app/registry/configure/edit-local-module.png" resize="600x" declaredimensions=true alt="The edit a local module pane with an example name and executable path">}}
...
You can send commands to the base according to the Viam [base API](/machine/components/base/#api):
...
Now that you've configured a modular resource, test it with the [**CONTROL** tab](/app/fleet/control/) and program it with the [Viam SDKs](/program/apis/).
