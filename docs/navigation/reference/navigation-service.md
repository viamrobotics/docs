...
| ------------------------------- | ------ | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `store`                         | object | `{"type": "memory"}` | Where to store waypoints. See [Store configuration](#store-configuration). Defaults to the in-memory store if omitted.                                                                                                                                                    |
...
| `replan_cost_factor`            | float  | `1.0`                | Multiplier applied to the current plan's cost when evaluating whether to replan after deviation. Higher values prefer the current plan (less eager to replan); lower values replan more aggressively. Must be non-negative. Omitting or setting to zero uses the default. |
...
{{< alert title="The built-in motion service does not support MoveOnGlobe" color="caution" >}}
