...
| `turn_on_hotspot_if_wifi_has_no_internet` | boolean | Optional | Controls whether or not a Wi-Fi connection without Internet access is considered **online**. When enabled, Wi-Fi connections without Internet access are considered **offline**. After `offline_before_starting_hotspot_minutes` minutes offline, your device will begin broadcasting a hotspot. Default: `false` (Wi-Fi connections without Internet access are considered online). |
