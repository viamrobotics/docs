...
| `offline_before_starting_hotspot_minutes` | integer | Optional | Amount of time the device will spend searching for the specified Wi-Fi network, in minutes, before the machine begins searching for an alternate network or broadcasting a wireless hotspot. Default: `2`. |
...
| `turn_on_hotspot_if_wifi_has_no_internet` | boolean | Optional | Controls whether or not a Wi-Fi connection without Internet access is considered 'online'. If enabled, if the Wi-Fi network provided during initial provisioning does not provide a connection to the Internet, the device will wait `offline_before_starting_hotspot_minutes` minutes. If the Wi-Fi network still does not provide a connection to the internet after this time has passed, your device will search for other Wi-Fi networks configured in `additional_networks`. If those networks cannot be found and the configuration includes a hotspot, the device will begin hosting a hotspot. Once a device connects to an alternate network or begins hosting a hotspot, the device will not attempt to reconnect to the original Wi-Fi network provided during intial provisioning until after a reboot. Default: `false`. |
