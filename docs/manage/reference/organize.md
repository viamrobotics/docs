...
Yes, organization owners and location owners can move machines between locations within their organization using the web UI.
...
1. Review the warnings about the implications of moving the machine.
1. Confirm the move.
...
- **Permission changes**: Access permissions will be updated. Current users may lose access, and users with access to the new location will gain access to the machine.
- **Data access**: Users in the new location cannot access historical data from when the machine was in the previous location.
- **Query impacts**: Location-specific or machine-specific data queries may return different results.
...
For more details, see [Move machines between locations](/manage/fleet/move-machines/).
