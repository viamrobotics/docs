...
| `control_frequency_hz` | float | Optional | Adjusts the frequency that the base control loop runs at. Increasing the frequency will generally result in a better controller, provided the movement sensors can support the higher frequency. The default base control loop frequency is 10Hz. |
