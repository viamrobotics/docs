...
There are currently mappings for a wired XBox 360 controller, and wireless XBox Series X|S, along with the 8bitdo Pro 2 bluetooth gamepad (which I definitely recommend as it works great with the Raspberry Pi.) The XBox controllers emulate an XBox 360 gamepad when in wired mode, as does the 8bitdo, so I presume this to be somewhat standard. Because of that, any unknown gamepad will be be mapped that way. If you have another controller though, feel free to add mappings, or ask me (James) for help doing so.


# Sample (Input Tester)
See samples/intputtest/cmd.go and/or build it with "make gamepad" from the root of core. Run it with the included gamepad_server.json for a directly connected gamepad. Optionally, on a second machine, run the same sample with gamepad_client.json (after modifying the remote host in the file to point to the system running the server.)
...
Note that the name MUST BE EXACTLY "WebGamepad" and will not work if another name is used. Hopefully this limitation will be removed as Core moves to v2.
...
# Mux
The Mux input controller simply combines one or more other controllers (sources) into a single virtual controller. This allows control from different locations (such as the web and a locally connected gamepad) or combining different controllers into one. (E.g. a joystick could be added to a numpad.)
