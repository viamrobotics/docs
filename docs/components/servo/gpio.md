...
| `frequency_hz` | int | Optional | The frequency of [PMW](/components/board/#getpwm) pulses sent to the servo, in Hertz (*Hz*). <br> Default = [`300`] <br> Range = [`0`, `450`] |
| `pwm_resolution` | int | Optional | The resolution of the [PMW](/components/board/#getpwm) driver (for example, the number of ticks for a full period). If not specified or given as `0`, the driver will attempt to estimate the resolution. |
| `min_width_us` | int | Optional | Override the safe minimum [PMW](/components/board/#getpwm) pulse width in microseconds. <br> Range > `450` |
| `max_width_us` | int | Optional | Override the safe maximum [PMW](/components/board/#getpwm) pulse width in microseconds. <br> Range < `2500` |
