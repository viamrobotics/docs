...
| `max_rpm` | float64 | Optional | Set a limit on maximum revolutions per minute that the motor can be run at. |
| `max_acceleration_rpm_per_sec` | float64 | Optional | Set the maximum revolutions per minute (RPM) per second acceleration limit. |
