...
`home_rpm` | integer | Speed in revolutions per minute that the motor will turn when executing a Home() command (using  DoCommand() since Home() is part of the DMC API but is not part of the built-in Viam motor API).
