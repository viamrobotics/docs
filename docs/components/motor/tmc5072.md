...
`home_rpm` | float64 | Speed in revolutions per minute that the motor will turn when executing a Home() command (via DoCommand()).
...
The `TMC5072` model supports an additional method that is not part of the standard Viam motor API (passed through DoCommand()):
