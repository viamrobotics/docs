...
| `spi_bus` | string | **Required** | The board [SPI bus](/components/board/#spi-bus) over which the TMC chip communicates with the board. |
...
| `index` | int | **Required** | The index of the [SPI bus](/components/board/#spi-bus). For example, on a Raspberry Pi a user could use SPI bus `0` or `1` |
