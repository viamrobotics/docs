...
| `board` | string | Optional | The `name` of the [board](/components/board/) to which the device is wired. Only needed if you've configured any [interrupt](#digital_interrupts) functionality. |
...
| `interrupt_pin`     | string | **Required** | The `name` of the [digital interrupt](#digital_interrupts) you configured for the pin on the [board](/components/board/) wired to the `accelerometer_pin`. |
