...
| `optimize_every_n_nodes` | How many trajectory nodes are inserted before the global optimization is run. | Optional | `3` | Setting to `0` will disable global SLAM, using local SLAM only. |
| `num_range_data` | Number of measurements in each submap. | Optional | `30` | |
| `missing_data_ray_length` | Replaces the length of ranges that are further than `max_range` with this value. | Optional | `25` | Should be the same as `max_range`. |
...
| `occupied_space_weight` | Emphasis to put on scanned data points between measurements. Higher values make it harder to overwrite prior scanned points. | Optional | `20.0` | Weights are all relative. |
| `translation_weight` | Emphasis to put on expected translational change from pose extrapolator data between measurements. Higher values make it harder for scan matching to translate prior points. | Optional | `10.0` | Weights are all relative. |
| `rotation_weight` | Emphasis to put on expected rotational change from pose extrapolator data between measurements. Higher values make it harder for scan matching to rotate prior points. | Optional | `1.0` | Weights are all relative. |
