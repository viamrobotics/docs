...
* Translation: The coordinates that the origin of this component's reference frame has within its parent's reference frame (these units are in millimeters)
    - keep in mind that +X is forward, +Y is right, and +Z is up
...
* Orientation: The rotation that when applied to the axes of the parent's reference frame yields the axes of the component's reference frame
    - while this representation appears to represent an R4 Axis Angle, it is actually our own [orientation vector](https://docs.viam.com/appendix/orientation-vector/) format (the X, Y, and Z components are in millimeters, but the resulting vector is automatically normalized to a unit vector).
    - in the UI the type option currently controls whether to supply theta in degrees or radians for the purposes of configuration, but note that *the orientation vector stored and returned by the frame system will be in radians*.
...
* Viam creates an additional static reference frame whose translation and orientation relative to its parent is provided by the user in configuration. Viam names this reference frame with the component name and the suffix *"_offset"*. For example, "right-arm_offset".
