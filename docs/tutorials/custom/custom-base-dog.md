...
You can configure a [_process_](/build/configure/#add-a-process) to automatically start the server on boot so it is ready to receive commands from `viam-server`.
