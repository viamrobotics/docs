...
The [base component type](/platform/build/configure/components/base/) is useful for controlling mobile robots because it gives users intuitive steering controls to use in code as well as from the [Viam app](https://app.viam.com/) remote control interface.
...
1. Follow the steps in our [Raspberry Pi Setup Guide](/platform/get-started/installation/prepare/rpi-setup/) to install Raspberry Pi OS Lite (64 bit).
...
1.  [Install `viam-server` and connect your robot to the Viam app](/platform/get-started/installation/#install-viam-server).
...
8. Enable I<sup>2</sup>C per [the instructions in the Raspberry Pi Setup Guide](/platform/get-started/installation/prepare/rpi-setup/#enable-communication-protocols).
...
Now that the Freenove server is set up, you will follow the [process for creating modular resources](/platform/registry/create/).
...
When you send a command to the robot using the Viam [base API](/platform/build/configure/components/base/#api), you need a way to pass the corresponding command to the Freenove dog server.
...
It contains stubs of all the [base API methods](/platform/build/configure/components/base/#api), but you need to modify these method definitions to actually send commands to the robot dog.
...
Feel free to tweak the specific contents of each of the [base method definitions](/platform/build/configure/components/base/#api) to do things like make the dog move faster.
...
You can find more information in [the relevant section of the modular resource documentation](/platform/registry/).
...
Configure the ribbon camera on the dog as a `webcam` following our [webcam documentation](/platform/build/configure/components/camera/webcam/).
...
You can configure a [_process_](/platform/build/configure/configuration/#processes) to automatically start the server on boot so it is ready to receive commands from `viam-server`.
...
You could also use the Viam [vision service](/platform/build/configure/services/vision/) with the robot dog's [camera component](/platform/build/configure/components/camera/).
