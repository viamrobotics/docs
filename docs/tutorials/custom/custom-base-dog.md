...
The [base component type](/hardware/common-components/add-a-base/) is useful for controlling mobile robots because it gives users intuitive steering controls to use in code as well as from the machine's **CONTROL** tab.
...
Now that the Freenove server is set up, you will follow the [process for creating modular resources](/build-modules/).
...
You can find more information in [Create a module](/build-modules/).
...
Configure the ribbon camera on the dog as a `webcam` following our [webcam documentation](/hardware/common-components/add-a-camera/).
...
You could also use the Viam [vision service](/vision/configure/) with the robot dog's [camera component](/hardware/common-components/add-a-camera/).
