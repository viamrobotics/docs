...
One of the components you need to configure is called a [base](/hardware/common-components/add-a-base/), which allows you to control a mobile robot using [commands](https://python.viam.dev/autoapi/viam/components/base/index.html#package-contents) like "move_straight", "spin", "set_velocity" and "stop".
...
For many robotic rovers you can use the [`wheeled`](/hardware/common-components/add-a-base/) base model.
...
- <a href="https://a.co/d/bxEdcAT" target="_blank">Raspberry Pi with microSD card</a>, with `viam-server` installed per [our Raspberry Pi setup guide](/reference/device-setup/rpi-setup/).
...
See [Start a new machine](/foundation/) for platform requirements and instructions.
...
For controlling a mobile robot's movements, the [base component](/hardware/common-components/add-a-base/) exposes a useful interface.
...
The [base](/hardware/common-components/add-a-base/) component exposes an API for controlling a mobile robot’s movements.
...
Since you are using the default Viam API for a [base](/hardware/common-components/add-a-base/), the {{< glossary_tooltip term_id="api-namespace-triplet" text="API namespace triplet" >}} is:
`rdk:component:base`.
...
1. Enter the module's [executable path](/build-modules/).
...
1. On the **Create** menu for a **Local component**:
   ...
   - Select the type of modular resource provided by your module: [base](/hardware/common-components/add-a-base/), from the dropdown menu.
...
For more information on modules and how they work, see the [Create a module](/build-modules/).
...
If your rover has a [`camera`](/hardware/common-components/add-a-camera/) or a [`movement_sensor`](/hardware/common-components/add-a-movement-sensor/), you can try the following tutorials:
