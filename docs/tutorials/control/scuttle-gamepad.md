...
Plug the gamepad Bluetooth dongle into a USB port on the rover's [board](/platform/build/configure/components/board/).
...
Configure a [gamepad](/platform/build/configure/components/input-controller/gamepad/):
...
If instead of using the config builder, you prefer to write raw JSON, switch to [**Raw JSON** mode](/platform/build/configure/configuration/#the-config-tab) on the **Config** tab.
Inside the `components` array of your config, add the following configuration for your [gamepad](/platform/build/configure/components/input-controller/gamepad/):
...
To link the controller's input to the base functionality, you need to configure the [base remote control service](/platform/build/configure/services/base-rc/):
