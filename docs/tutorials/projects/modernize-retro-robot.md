...
Let's get started with wiring the left and right motors to a motor controller and Raspberry Pi - this will allow your Omnibot to be controlled with Viam as a [wheeled base](/platform/build/configure/components/base/wheeled/).
...
Go to the **Setup** tab of your new robot's page and follow the steps [to install `viam-server` on your Raspberry Pi](/platform/get-started/installation/).
...
2. **Add the left motor.**
   ...
   Click **Create component** to add the [motor component](/platform/build/configure/components/motor/) on the left side of the robot base.
...
3. **Add the right motor**
   ...
   For your right base [motor component](/platform/build/configure/components/motor/), select the type `motor`, and select the `gpio` model.
...
4. **Add the base**
   ...
   Configuring a [base component](/platform/build/configure/components/base/) allows you to create an interface to control the movement of MAIV withing needing to send individual motor commands.
...
On the [`Raw JSON` tab](/platform/build/configure/configuration/#the-config-tab), replace the configuration with the following JSON configuration for your board, left motor, right motor, and base:
...
Navigate to your robot's [Control](/platform/fleet/robots/#control) tab.
...
To create your [motor component](/platform/build/configure/components/motor/), select the type `motor`, and select the `gpio` model.
...
{{< tabs >}}
{{% tab name="Builder UI" %}}
Add the [camera component](/platform/build/configure/components/camera/) to your robot by navigating to the **Config** tab of your robot's page in [the Viam app](https://app.viam.com).
