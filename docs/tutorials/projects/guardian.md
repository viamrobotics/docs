...
Adding a bit of [machine learning](/platform/build/configure/services/ml/), you can then make the guardian detect objects or people or pets and follow them around by rotating its head.
...
In this tutorial, I will walk you through the steps to build your own functional guardian with a [servo](/platform/build/configure/components/servo/), a [camera](/platform/build/configure/components/camera/), some LEDs and the [ML Model service](/platform/build/configure/services/ml/) and [vision service](/platform/build/configure/services/vision/).
...
- [`viam-server`](/platform/get-started/installation/#install-viam-server): Follow the [installation instructions](/platform/get-started/installation/#install-viam-server) to install `viam-server` on your Raspberry Pi.
...
Go to the **Setup** tab of your new robot's page and follow the steps [to install `viam-server` on your computer](/platform/get-started/installation/).
...
1. **Add the board.**
   ...
   Enter `local` as the name for your [board component](/platform/build/configure/components/board/), then click **Create**.

2. **Add the camera.**

   Click **Create Component** to add the [camera](/platform/build/configure/components/camera/).
...
3. **Add the servo.**
   ...
   Enter `servo` as the name for your [servo component](/platform/build/configure/components/servo/), then click **Create**.
...
On the [`Raw JSON` tab](/platform/build/configure/configuration/#the-config-tab), replace the configuration with the following JSON configuration for your board, your camera, and your servo with its PWM wire wired to {{< glossary_tooltip term_id="pin-number" text="pin number" >}} `12`:
...
Navigate to your [robot's Control tab](/platform/fleet/robots/#control) to test your components.
...
Then test the components on the [robot's Control tab](/platform/fleet/robots/#control) again to ensure everything still works.
...
You can also [train your own custom model](/platform/ml/train-model/) based on images from your robot but the provided Machine Learning model is a good one to start with.
...
The [ML model service](/platform/build/configure/services/ml/) allows you to deploy the provided machine learning model to your robot.
...
Next, add a [detector](/platform/build/configure/services/vision/detection/) as a vision service to be able to make use of the ML model.
...
Enter `transform_cam` as the name for your [transform camera](/platform/build/configure/components/camera/transform/), then click **Create**.
...
Next, on the [**Raw JSON** tab](/platform/build/configure/configuration/#the-config-tab), replace the configuration with the following configuration which configures the [ML model service](/platform/build/configure/services/ml/), the [vision service](/platform/build/configure/services/vision/), and a [transform camera](/platform/build/configure/components/camera/transform/):
...
Navigate to your [robot's Control tab](/platform/fleet/robots/#control) to test the transform camera.
...
The `mlmodel` extra includes additional dependency support for the [ML (machine learning) model service](/platform/build/configure/services/ml/).
...
You can also configure Viam to automatically run your code as a [process](/platform/build/configure/configuration/#processes).
...
[`ssh` into your Pi](/platform/get-started/installation/prepare/rpi-setup/#connect-with-ssh) and install `pip`:
...
Of course, you're free to adapt the code to make it do something else, add more LEDs, or even [train your own custom model](/platform/ml/train-model/) to use.
