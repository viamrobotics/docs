...
- master the art of controlling your robot with our [motion service](/reference/apis/services/motion/) using the [Viam Python SDK](https://python.viam.dev/)
...
If you have two computers within the _same machine_, you can use one as the main part and [connect the other to it as a sub-part](/reference/).
This is the approach this tutorial follows: you'll run the [motion planning service](/reference/apis/services/motion/) on a laptop and connect that laptop as a sub-part to your machine.
...
For more information about parts, see [Machine Architecture: Parts](/reference/).
...
Add your [board](/hardware/common-components/add-a-board/) with type `board` and model `viam:raspberry-pi:rpi` if you are using a Raspberry Pi 4, Raspberry Pi 3 or Raspberry Pi Zero 2 W.
...
Click **Create component** and add your [gripper](/hardware/common-components/add-a-gripper/).
...
The claw game machine will use the [motion service](/reference/apis/services/motion/) to plan its movements.
...
See the [frame system documentation](/reference/) for more information.
{{< /alert >}}
...
Then we define the [constraints](https://python.viam.dev/autoapi/viam/proto/service/motion/index.html#viam.proto.service.motion.Constraints) - in this case we are using an [orientation constraint](/motion-planning/).
...
Lastly, the code defines the functions `move_absolute()`, `home()`, `move_to_offset()` and `move_z()`, which construct new pose requests to send to the [motion service](/reference/apis/services/motion/).
...
- Test, configure, and control a robot arm using Viam’s [motion service](/reference/apis/services/motion/), the [Viam Python SDK](https://python.viam.dev/), and the [web UI](https://app.viam.com).
- Design your own custom interface using the [Viam TypeScript SDK](https://ts.viam.dev/).

For some next steps, you could:

- Use the advanced interface included in the project repository to leverage the [motion service](/reference/apis/services/motion/) for larger, more complex arm movement within the enclosure.
- Add a camera and use the [vision service](/vision/configure/) to add color detection, or use an [ML model](/vision/configure/) to determine grab success rate and create a score counter.
