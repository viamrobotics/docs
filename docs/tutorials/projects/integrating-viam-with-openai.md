...
By combining ChatGPT with the Viam platform’s built-in [computer vision service](/vision/configure/), ML model support, and [locomotion](/hardware/common-components/add-a-base/), you can within a few hours create a basic companion robot that:
...
We'll use a [servo](/hardware/common-components/add-a-servo/) in this project to indicate emotion, by rotating the servo to a position that shows a happy, sad, or angry emoji.
...
To configure your [servo](/hardware/common-components/add-a-servo/), go to your rover's **CONFIGURE** tab.
...
- Enter `8` for `pin`.
- Select the name of your [board](/hardware/common-components/add-a-board/) for the `board` attribute: in this case, `local`.
