...
By the end of this tutorial, you will have learned how to wire a motor and motor driver to a Raspberry Pi, install [`viam-server`](/platform/get-started/installation/) on your Raspberry Pi, and how to configure your robot in the Viam app to create your own Lazy Susan for your dinner table!

You will also learn to take your creation a step further and fine-tune the controls of your Lazy Susan using the [Python motor API methods](/platform/build/configure/components/motor/#api).
...
If you have not already done so, follow the [Raspberry Pi Setup Guide](/platform/get-started/installation/prepare/rpi-setup/) to prepare your Pi to connect your robot to the Viam app.
...
2. **Configure the motor**
   ...
   Next, add a [motor component](/platform/build/configure/components/motor/):
   ...
   After creating your motor component, fill in the [required attributes for a gpio motor](/platform/build/configure/components/motor/gpio/):
...
On the [`Raw JSON` tab](/platform/build/configure/configuration/#the-config-tab), replace the configuration with the following JSON configuration for your board and motor:
...
[SetPower](/platform/build/configure/components/motor/#setpower) adjusts speed by adjusting the power to the motor.
...
Reference the [SetPower API Method example](/platform/build/configure/components/motor/#setpower) for more information.

[GoFor](/platform/build/configure/components/motor/#gofor) allows you to control how fast the motor spins by allowing you to set the revolutions per minute (or RPMs).
...
However, there is also an API method you can use to [Stop](/platform/build/configure/components/motor/#stop) the motor!
...
If you would like to have the same effect but use even less code, you can use the [GoFor API Method](/platform/build/configure/components/motor/#gofor).
...
- An [input controller component](/platform/build/configure/components/input-controller/) so you can turn and stop the Lazy Susan with a gamepad.
- A [camera component](/platform/build/configure/components/camera/) and an [ML model](/platform/build/configure/services/ml/) that recognizes your friends' faces and stops the Lazy Susan precisely where they can reach their favorite food, or takes their photo as it rotates, so you all can have candid memories.
- A [sensor component](/platform/build/configure/components/sensor/), so your Lazy Susan only spins when it detects movement around it.
