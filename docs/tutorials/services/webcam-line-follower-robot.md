...
**Goal**: To make a wheeled robot follow a colored line along the floor using a webcam and the Viam <a href="/machine/services/vision/#detections">vision service color detector</a>.

**What you will learn**:

- How to use the [vision service](/services/vision/)'s [color detectors](/machine/services/vision/color_detector/)
...
| **A wheeled [base component](/machine/components/base/)**: This tutorial uses a [SCUTTLE robot](https://www.scuttlerobot.org/shop/), but any other wheeled base works as long as it can carry the board and camera, and is capable of turning in place. | $99+ |
...
1. **Add the board.**
   ...
   Enter `local` as the name of your [board component](/machine/components/board/), then click **Create**.

2. **Add the motors.**

   Click **Create component**.
   Select the type `motor`, and select the `gpio` model.
   Enter `leftm` as the name of your [motor component](/machine/components/motor/), then click **Create** and fill in the appropriate properties for your motor.
...
3. **Add the base.**
   ...
   Enter `scuttlebase` as the name for your [base component](/machine/components/base/), then click **Create** and select the motors.

4. **Add the camera.**

   Create a [camera component](/machine/components/camera/) with the name `my_camera`, the type `camera` and the model `webcam`.
...
On the [`Raw JSON` tab](/machine/configure/#the-configure-tab), replace the configuration with the following JSON configuration for your board, your motors, your base, and your camera:

{{< alert title="Note" color="note" >}}
Your `"video_path"` value may be different.
To find yours, follow [these instructions](/machine/components/camera/webcam/#using-video_path).
{{< /alert >}}
...
Navigate to your [machine's **Control** tab](/app/fleet/control/) to test your components.
...
You'll use the [vision service color detector](/machine/services/vision/color_detector/) to programmatically identify the line to follow.
...
If you'd like to see the bounding boxes that the color detector identifies, you'll need to configure a [transform camera](/machine/components/camera/transform/).
...
Add a [transform camera](/machine/components/camera/transform/) with type `camera` and model `transform`.
...
On the [`Raw JSON` tab](/machine/configure/#the-configure-tab), replace the configuration with the following JSON configuration which adds the configuration for the vision service and the transform camera:
...
Navigate to your [machine's **Control** tab](/app/fleet/control/) to test the transform camera.
...
- Add a [`saturation_cutoff_pct` or a `value_cutoff_percent`](/machine/services/vision/color_detector/) to your vision service parameters.
