...
Once you have [configured data query](/app/data/query/#configure-data-query) for your organization's data store, you can visualize your data from a variety of third-party tools, including Grafana.
...
Only components that capture tabular data, such as [sensor](/machine/components/sensor/) readings or other time-series data, support data visualization.

You can visualize both the captured tabular data itself as well as its metadata, including machine ID, organization ID, and [tags](/app/data/dataset/#image-tags).
...
For general guidance appropriate for any third-party visualization tool, see [Visualize data](/app/data/visualize/).
{{% /alert %}}
...
- A machine with [connected to the Viam app](/app/fleet/machines/#add-a-new-machine).

- A configured [sensor component](/machine/components/sensor/), such as the [ultrasonic](/machine/components/sensor/ultrasonic/) sensor, that reports tabular data.
...
You can manage how your machine works with data by using the [data management service](/app/data/).

The data management service has two parts: [data capture](/app/data/capture/) and [cloud sync](/app/data/cloud-sync/).
...
For this tutorial, you will configure data capture for a [sensor](/machine/components/sensor/) component, gathering sensor readings to later visualize in Grafana.
...
Configuring data query allows you to directly [query your data using the Viam app](/app/data/query/#query-tabular-data-in-the-viam-app) or [query your data from a compatible client](/app/data/query/#query) (such as `mongosh`), but also allows Grafana to access that data and visualize it.

To enable data query:

1. Follow the steps to [configure data query](/app/data/query/#configure-data-query).
...
1. Enter the following information in the configuration UI for that plugin:
   ...
   - **Connection string**: Enter the following connection string, substituting the hostname returned from the previous section for `<MONGODB-ATLAS-DF-HOSTNAME>` and the desired database name to query for `<DATABASE-NAME>`:
     ...
     You must have followed the steps under [configure data query](/app/data/query/#configure-data-query) previously in order for this URL to be valid.
   ...
   - **Credentials: Password**: Enter the password you provided when you [configured data query](/app/data/query/#configure-data-query) previously.
...
This connection allows Grafana to access all synced tabular data under your [organization](/app/fleet/organizations/), from any machine.
...
- how to use the [data management](/app/data/) service to capture data from your machine and sync it to the Viam app
- how to [enable data query access](/app/data/query/#configure-data-query) to your synced data
