...
With Viam you can move individual components, like [arms](/hardware/common-components/add-an-arm/), by issuing commands like `MoveToPosition` or `MoveToJointPosition`.
The [motion service](/reference/apis/services/motion/) enables you to do much more sophisticated movement involving one or many components of your robot.
...
Code examples in this tutorial use a [UFACTORY xArm 6](https://www.ufactory.us/product/ufactory-xarm-6), but you can use any [arm model](/hardware/common-components/add-an-arm/).
...
1. [Configure an Arm](/motion-planning/)
2. [Configure your frame system](/motion-planning/)
3. [Configure components attached to your arm](/motion-planning/)

## Configure a robot

Use the robot configuration from the [prerequisite guide](/motion-planning/) for this tutorial as well.
...
The Motion service has a method that can get the _pose_ of a component relative to a [_reference frame_](/reference/).
...
One device that is very commonly attached to the end of a robot arm is a [_gripper_](/hardware/common-components/add-a-gripper/).
...
This is possible because the motion service considers other components of the robot (through the [frame system](/reference/)) when calculating how to achieve the desired motion.
