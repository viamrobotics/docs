...
The [navigation service](/reference/apis/services/navigation/) allows you to queue up user-defined waypoints for your machine to move to in the order that you specify.
...
1. **A base**
   ...
   We used [a LEO rover](https://www.leorover.tech/shop?gclid=CjwKCAjw38SoBhB6EiwA8EQVLiDwUFEYgLxaRd1-TiTyLfifIAHs9iD6YnvdW6M-3rXruHOrzfTL2RoCD1AQAvD_BwE), configured as a [`wheeled` base](/hardware/common-components/add-a-base/), but you can use whatever model of rover base you have on hand:
...
2. **A movement sensor with GPS position, compass heading, and angular and linear velocity readings**
   ...
   1. A [SparkFun GPS-RTK-SMA Breakout](https://www.sparkfun.com/products/16481) [movement sensor](/hardware/common-components/add-a-movement-sensor/) configured as a [`gps-nmea-rtk-serial`](https://app.viam.com/module/viam/gps) model, providing GPS position and compass heading measurements.
   2. A [`wheeled-odometry`](/hardware/common-components/add-a-movement-sensor/) model gathering angular and linear velocity information from the [encoders](/hardware/common-components/add-an-encoder/) wired to our base's [motors](/hardware/common-components/add-a-motor/).
   3. A [`merged`](/hardware/common-components/add-a-movement-sensor/) model aggregating the readings together for the navigation service to consume.

   You can use any combo of movement sensors you want as long as you are getting all the types of measurements required.
   See [the navigation service](/reference/apis/services/navigation/) for more info on movement sensor requirements.
...
Before you start, make sure to create a machine on [Viam](https://app.viam.com) and [install `viam-server`](/foundation/) on your robot.
...
First, configure the [board](/hardware/common-components/add-a-board/) local to your rover.
Follow [these instructions](/hardware/common-components/add-a-board/) to configure your board model.
...
2. Configure [digital interrupts](https://github.com/viam-modules/nvidia/blob/main/README.md#digital-interrupt-configuration) on your board to signal precise GPIO state changes to the [encoders](/hardware/common-components/add-an-encoder/) on your rover base.
...
Start by configuring the [encoders](/hardware/common-components/add-an-encoder/) and [motors](/hardware/common-components/add-a-motor/) of your encoded motor.

1. Follow the [encoder configuration instructions](/hardware/common-components/add-an-encoder/) to configure the left and right encoders of the wheeled base.
   We configured ours as [`incremental` encoders](/hardware/common-components/add-an-encoder/), as shown below:
   ...
   Refer to the [`incremental` encoder documentation](/hardware/common-components/add-an-encoder/) for attribute information.

2. Next, follow [these instructions](/hardware/common-components/add-a-motor/) to configure the left and right [motors](/hardware/common-components/add-a-motor/) of the `wheeled` base.
   We [configured ours as `gpio` motors](/hardware/common-components/add-a-motor/), as shown below:
...
Refer to the [`gpio` motor](/hardware/common-components/add-a-motor/) documentation for attribute information.

3. Finally, configure whatever rover you have as a [`wheeled`](/hardware/common-components/add-a-base/) model of base, bringing the motion produced by these motors together on one platform:
   ...
   - [Configure the frame system](/reference/) for this wheeled base so that the navigation service knows where it is in relation to the movement sensor.
   ...
   Refer to the [`wheeled` base configuration instructions](/hardware/common-components/add-a-base/) for attribute information.
...
2.  Configure a wheeled odometry movement sensor to provide angular and linear velocity measurements from the encoded motors on our base.
    ...
    Refer to [the `wheeled-odometry` movement sensor documentation](/hardware/common-components/add-a-movement-sensor/) for attribute information.
...
3.  Now that you've got movement sensors which can give you GPS position and linear and angular velocity readings, configure a `merged` movement sensor to aggregate the readings from our other movement sensors into a singular sensor:
    ...
    Refer to [the `merged` movement sensor documentation](/hardware/common-components/add-a-movement-sensor/) for attribute information.

    - Make sure your `merged` movement sensor is configured to gather `"position"` readings from the `gps` movement sensor.
    - [Configure the frame system](/reference/) for this movement sensor so that the navigation service knows where it is in relation to the base.
...
1. Select **JSON** mode.
   ...
   Attribute information is available in [the navigation service documentation](/reference/apis/services/navigation/).
...
For more detailed information see [the navigation service](/reference/apis/services/navigation/).
...
First, configure a depth [camera](/hardware/common-components/add-a-camera/) that your robot can sense how far away it is from obstacles.
