...
One key feature of Viam is [the navigation service](/platform/build/configure/services/navigation/), the stateful definition of Viam's [motion service](/platform/build/configure/services/motion/).
...
1. **A base**
   ...
   We used [a LEO rover](https://www.leorover.tech/shop?gclid=CjwKCAjw38SoBhB6EiwA8EQVLiDwUFEYgLxaRd1-TiTyLfifIAHs9iD6YnvdW6M-3rXruHOrzfTL2RoCD1AQAvD_BwE), configured as a [`wheeled` base](/platform/build/configure/components/base/wheeled/), but you can use whatever model of rover base you have on hand:
...
2. **A movement sensor with GPS position, compass heading, and angular and linear velocity readings**
   ...
   1. A [SparkFun GPS-RTK-SMA Breakout](https://www.sparkfun.com/products/16481) [movement sensor](/platform/build/configure/components/movement-sensor/) configured as a [`gps-nmea-rtk-serial`](/platform/build/configure/components/movement-sensor/gps/gps-nmea-rtk-serial/) model, providing GPS position and compass heading measurements.
   2. A [`wheeled-odometry`](/platform/build/configure/components/movement-sensor/wheeled-odometry/) model gathering angular and linear velocity information from the [encoders](/platform/build/configure/components/encoder/) wired to our base's [motors](/platform/build/configure/components/motor/).
   3. A [`merged`](/platform/build/configure/components/movement-sensor/merged/) model aggregating the readings together for the navigation service to consume.

   You can use any combo of movement sensors you want as long as you are getting all the types of measurements required.
   See [the navigation service](/platform/build/configure/services/navigation/#requirements) for more info on movement sensor requirements.
...
Before you start, make sure to create a robot in [the Viam app](https://app.viam.com) and [install `viam-server`](/platform/get-started/installation/) on your robot.
...
If you are using different hardware, configure them according to the instructions for each [component](/platform/build/configure/components/).
{{< /alert >}}
...
First, configure the [board](/platform/build/configure/components/board/) local to your rover.
Follow [these instructions](/platform/build/configure/components/board/#supported-models) to configure your board model.
We used a [`jetson` board](/platform/build/configure/components/board/jetson/), but you can use any model of board you have on hand, as the [resource's API](/platform/build/configure/components/board/#api) is hardware agnostic.
...
2. Configure [digital interrupts](/platform/build/configure/components/board/#digital_interrupts) on your board to signal precise GPIO state changes to the [encoders](/platform/build/configure/components/encoder/) on your rover base.
   Copy and paste the following into your board's **Attributes** to add [digital interrupts](/platform/build/configure/components/board/#digital_interrupts) on pins `31`, `29`, `23`, and `21`:
...
Start by configuring the [encoders](/platform/build/configure/components/encoder/) and [motors](/platform/build/configure/components/motor/) of your encoded motor.

1. Follow [these instructions](/platform/build/configure/components/) to configure the left and right encoders of the wheeled base.
   We configured ours as [`incremental` encoders](/platform/build/configure/components/encoder/incremental/), as shown below:
   ...
   Assign the pins as the [digital interrupts](/platform/build/configure/components/board/#digital_interrupts) you configured for the board, and wire the encoders accordingly to pins {{< glossary_tooltip term_id="pin-number" text="numbered" >}} `31`, `29`, `23`, and `21` on your `local` board.
   Refer to the [`incremental` encoder documentation](/platform/build/configure/components/encoder/incremental/) for attribute information.

2. Next, follow [these instructions](/platform/build/configure/components/motor/#supported-models) to configure the left and right [motors](/platform/build/configure/components/motor/) of the `wheeled` base.
   We [configured ours as `gpio` motors](/platform/build/configure/components/motor/gpio/), as shown below:
   ...
   Refer to the [`gpio` motor](/platform/build/configure/components/motor/gpio/) documentation for attribute information.

3. Finally, configure whatever rover you have as a [`wheeled`](/platform/build/configure/components/base/wheeled/) model of base, bringing the motion produced by these motors together on one platform:
   ...
   - [Configure the frame system](/platform/build/configure/services/frame-system/#configuration) for this wheeled base so that the navigation service knows where it is in relation to the movement sensor.
   ...
   Refer to the [`wheeled` base configuration instructions](/platform/build/configure/components/base/wheeled/) for attribute information.
...
1.  Configure a GPS movement sensor so the robot knows where it is while navigating.
    ...
    We named ours `gps`.
    Refer to [the `gps-nmea-rtk-serial` movement sensor documentation](/platform/build/configure/components/movement-sensor/gps/gps-nmea-rtk-serial/) for attribute information.
...
2.  Configure a wheeled odometry movement sensor to provide angular and linear velocity measurements from the encoded motors on our base.
    ...
    Refer to [the `wheeled-odometry` movement sensor documentation](/platform/build/configure/components/movement-sensor/wheeled-odometry/) for attribute information.
...
3.  Now that you've got movement sensors which can give you GPS position and linear and angular velocity readings, configure a `merged` movement sensor to aggregate the readings from our other movement sensors into a singular sensor:
    ...
    Refer to [the `merged` movement sensor documentation](/platform/build/configure/components/movement-sensor/merged/) for attribute information.

    - Make sure your `merged` movement sensor is configured to gather `"position"` readings from the `gps` movement sensor.
    - [Configure the frame system](/platform/build/configure/services/frame-system/#configuration) for this movement sensor so that the navigation service knows where it is in relation to the base.
...
4. Select **Raw JSON** mode.
   ...
   Attribute information is available in [the navigation service documentation](/platform/build/configure/services/navigation/#configuration).
...
For more detailed information see [the navigation service](/platform/build/configure/services/navigation/#configuration).
...
If you want to do add waypoints programmatically, use the service's [API method `AddWaypoint()`](/platform/build/configure/services/navigation/#addwaypoint):
...
To start navigating, set your service to `MODE_WAYPOINT` with the service's [API method `SetMode()`](/platform/build/configure/services/navigation/#setmode):
...
First, configure a depth [camera](/platform/build/configure/components/camera/) that your robot can sense how far away it is from obstacles.
...
You can alternatively use an [`ultrasonic` sensor](/platform/build/configure/components/sensor/ultrasonic/) configured as a camera.
Attribute information for an `ultrasonic` [camera](/platform/build/configure/components/camera/) is the same as [for a sensor](/platform/build/configure/components/sensor/ultrasonic/).

{{< /alert >}}

If you want the robot to be able to automatically detect obstacles in front of it, [configure a Vision service segmenter](/platform/build/configure/services/vision/segmentation/).
For example, [configure](/platform/build/configure/services/vision/segmentation/#configure-an-obstacles_depth-segmenter) the Vision service model [`obstacles_depth`](/platform/build/configure/services/vision/segmentation/#configure-an-obstacles_depth-segmenter) to detect obstacles in front of the robot.
Then, use one of [Viam's client SDKs](/platform/build/program/) to automate obstacle avoidance with the navigation service like in the following Python program:
