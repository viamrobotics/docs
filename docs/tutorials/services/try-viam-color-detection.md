...
In this tutorial you will learn how to configure a color detector with the vision service and leverage that detector with a [transform camera](/platform/build/configure/components/camera/transform/) to detect the color red.

The [vision service](/platform/build/configure/services/vision/) enables a robot to use its cameras to see and interpret the world around it.
...
You can follow this tutorial with a [rented Viam Rover](https://app.viam.com/try) or with [your own Viam Rover](/platform/get-started/try-viam/rover-resources/).
...
Be aware that if you are running out of time during your rental, you can [extend your rover rental](/platform/get-started/try-viam/reserve-a-rover/#extend-your-reservation) as long as there are no other reservations.
{{< /alert >}}
...
- If you are running this tutorial with a [rented Viam Rover](https://app.viam.com/try), enable both provided cameras: the front-facing camera and the overhead cam.
  ...
  You can also view and control the camera streams from the [individual camera component panels](/platform/get-started/try-viam/try-viam-tutorial/#camera-control).

- If you are running this tutorial on [your own Viam Rover](/platform/get-started/try-viam/rover-resources/), enable the front facing camera.
  If you are using the `ViamRover` [fragment](/platform/get-started/try-viam/rover-resources/rover-tutorial-fragments/) with your rover, the front facing camera is named `cam` and can be enabled in the `viam_base` component panel under the **Control** tab.
...
Navigate to your robot's **Config** tab on the [Viam app](https://app.viam.com/robots) and configure your [vision service color detector](/platform/build/configure/services/vision/detection/):
...
![The vision service configuration panel showing the color set to a reddish color, the hue tolerance set to 0.06, and the segment size set to 100.](/platform/get-started/try-viam/try-viam/vision-service-config.png)
...
You cannot interact directly with the [vision service](/platform/build/configure/services/vision/).
...
Viam [camera](/platform/build/configure/components/camera/) components can be physical like the one already configured on the rover, or virtual.
A virtual [_transform camera_](/platform/build/configure/components/camera/transform/) transforms the output from a physical camera.
