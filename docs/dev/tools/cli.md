...
viam machines part history --part=<part id> [--filter-by-email=<email>]
```
...
# View the configuration history for a machine part:
viam machines part history --part=123

# View configuration history filtered by a specific user:
...
| `history` | View the configuration history for a machine part. Output shows index, local timestamp, and editor email for each history entry. |
| `--help` | Return help. |
...
| `--filter-by-email` | Filter history results to only show entries made by the specified email address. | `part history` | Optional |
