...
| `firebase-config set` | Upload a Firebase config JSON file for push notifications. Organization owner only. | - |
| `firebase-config read` | Read Firebase config metadata for an organization. Returns only the app ID for security. Organization owner only. | - |
...
| `--firebase-config-path` | Path to the Firebase config JSON file. | `firebase-config set` | **Required** |
