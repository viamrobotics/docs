...
For example, imagine a wheeled rover gets a [`SetPower()`](/machine/components/base/#setpower) command as the last input from a client before the connection to the machine is interrupted.
...
For more information, see [Client Sessions and Machine Network Connectivity](/program/connectivity/).
...
{{<imgproc src="/program/sessions/heartbeatwindow.png" alt="Heartbeat window configuration pane in Viam app" resize="400x" declaredimensions=true >}}
