...
After saving your [code sample](/program/#hello-world-the-connect-tab) and adding control logic with [Viam's SDKs](/program/apis/), run your program to control your Viam-connected machine.
...
{{<imgproc src="/program/remotely.png" resize="900x" declaredimensions=true alt="A client connecting remotely to a machine">}}
...
When you run code on one computer, creating a client [session](/program/apis/sessions/), the code running on that computer sends instructions to your machine's `viam-server` instance over the Internet.
...
When you use the connection code sample from the [**CONNECT** tab](/program/#hello-world-the-connect-tab), that code establishes a [client session](/program/apis/sessions/) that automatically uses the [most efficient route](/program/connectivity/) to send commands to your machine.
...
{{<imgproc src="/program/on-robot.png" resize="900x" declaredimensions=true alt="A client running on a machine">}}

The script you run on-machine is the same as the script you [run remotely](#run-code-remotely) or on a local network.
When the connection code from the [**CONNECT** tab's **Code sample** page](/program/#hello-world-the-connect-tab) executes, it creates a [client session](/program/apis/sessions/) connected to your machine using the [most efficient route](/program/connectivity/).
...
Find information on how to configure a process in [Processes](/machine/configure/processes/).
