...
| `disable_log_deduplication`         | boolean | `false` | Disable log deduplication for `viam-agent`. When enabled (default), identical consecutive log messages are collapsed into a single message with a repeat count. Set to `true` to see every log line individually. |
