...
The [`viam:find-webcams:webcam-discovery`](https://app.viam.com/module/viam/find-webcams) service helps you identify path options.
...
- Ensure that your Linux OS is able to access your camera:
  ...
  1.  Then, stop `viam-server`, and verify that your Linux OS is able to access that video device properly:
...
- Ensure that your camera uses a supported pixel format:
  ...
  1.  Then, run the following command:
      ...
      In order to use a camera device with Viam, it must support at least one of the pixel formats listed in [Using `format`](#using-format).
