...
For example, you can use a `fake` component as a placeholder while waiting on a hardware shipment, so that other components that depend on this motor (for example, a [base](/reference/components/base/)) won't fail to initialize, and your control code won't throw errors when it fails to find a physical motor connected to your machine.
...
| ---- | ---- | --------- | ----------- |
| `board` | string | Optional | `name` of the [board](/reference/components/board/) to which the motor driver is wired. |
...
| `encoder` | string | Optional | The name of an encoder attached to this motor. See [encoded motor](/reference/components/motor/encoded-motor/). *If an encoder is configured on a `fake` motor, `ticks_per_rotation` becomes required.* |
...
| Name | Type | Required? | Description |
| ---- | ---- | --------- | ----------- |
| `a` | string | Optional | See [Pin Information](#pin-information). Corresponds to "IN1" on many driver data sheets. {{< glossary_tooltip term_id="pin-number" text="Pin number" >}} of the GPIO pin this is wired to on the [board](/reference/components/board/). |
| `b` | string | Optional | See [Pin Information](#pin-information). Corresponds to "IN2" on many driver data sheets. {{< glossary_tooltip term_id="pin-number" text="Pin number" >}} of the GPIO pin this is wired to on the [board](/reference/components/board/). |
