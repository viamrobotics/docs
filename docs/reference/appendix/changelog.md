...
To make it easier to iterate while training machine learning models from image data, you now train models from [datasets](/platform/data/dataset/).
...
For more information, see [Test classification models with existing images in the cloud](/platform/build/configure/services/vision/classification/#existing-images-in-the-cloud).
...
The [Cartographer-module](/platform/build/configure/services/slam/cartographer/) now runs in Viam's cloud for creating or updating maps.
...
The [Modular Registry](/platform/registry/) enables you to use, create, and share custom modules, extending the capabilities of Viam beyond the components and services that are natively supported.
...
You can use a [mobile application](/platform/fleet/#the-viam-mobile-app), available for download now in the [Apple](https://apps.apple.com/us/app/viam-robotics/id6451424162) and [Google Play](https://play.google.com/store/apps/details?id=com.viam.viammobile&hl=en&gl=US) app stores, to connect to and control your Viam-powered machines directly from your mobile device.

### Added: Power sensor component

You now have the capability to use a [power sensor component](/platform/build/configure/components/power-sensor/) to monitor the voltage, current, and power consumption within your machine's system.

### Added: Filter component's data before the cloud

Viam has written a module that allows you to filter data based on specific criteria before syncing it to [Viam's cloud](/platform/build/configure/services/data/#cloud-sync).
...
You can now use boards like the [Mediatek Genio 500 Pumpkin](https://ologicinc.com/portfolio/mediateki500/) that run Linux operating systems with the [`customlinux` board model](/platform/build/configure/components/board/customlinux/).
...
Arm models now support the [`GetKinematics` method](/platform/build/configure/components/arm/#getkinematics) in the arm API, allowing you to request and receive kinematic information.
...
[Session management](/platform/build/program/apis/sessions/) helps you to ensure safer operation of your robot when dealing with actuating controls.
Sessions are enabled by default, with the option to [disable sessions](/platform/build/program/apis/sessions/#disable-default-session-management).
...
This update enables you to implement custom models of an arm component as a [modular resource](/platform/registry/) by coding three endpoints of the [Arm API](/platform/build/configure/components/arm/#api):

- `getJointPositions`
- `movetoJointPositions`
- `GetKinematics`

Then, use the [motion planning service](/platform/build/configure/services/motion/) to specify poses, and Viam handles the rest.

For more information, see this [tutorial on creating a custom arm](/platform/registry/examples/custom-arm/).

### Added: Apply a crop transform to camera views

You can now apply a [crop transform](/platform/build/configure/components/camera/transform/) to the views of your connected cameras in the Viam app.
...
- Specify speed values when calling the `MovetoPosition` method on [Gantry components](/platform/build/configure/components/gantry/).
...
This update for object detection models [trained with the machine learning service](/platform/ml/train-model/) brings significant improvements, including:
...
You now have the capability to directly [train object detection models](/platform/ml/train-model/) in addition to image classification models from within the Viam app.
...
- **Owner**: These members can see and edit [every tab on the robot page](/platform/fleet/robots/#navigating-the-robot-page), as well as manage users in the app.
  This role is best for those on your team who are actively engineering and building robots.

- **Operator**: These members can only see and use the [remote control tab](/platform/fleet/robots/#control).
  This role is best for those on your team who are teleoperating or remotely controlling robots.

For more information about assigning permissions and collaborating with others on Viam, see [Fleet Management](/platform/fleet/).
...
- The ability to hide or display your robots' [secrets](/platform/build/program/apis/)
...
You can replace existing color detectors by [configuring new ones in the UI](/platform/build/configure/services/vision/detection/#configure-a-color_detector) or you can update the [Raw JSON configuration of your robots](/platform/build/configure/configuration/#the-config-tab):
...
You can replace existing TFLite detectors by [configuring new ones in the UI](/platform/build/configure/services/vision/detection/#configure-an-mlmodel-detector) or you can update the [Raw JSON configuration of your robots](/platform/build/configure/configuration/#the-config-tab):
...
You can replace existing TFLite classifiers by [configuring new ones in the UI](/platform/build/configure/services/vision/classification/#configure-an-mlmodel-classifier) or you can update the [Raw JSON configuration of your robots](/platform/build/configure/configuration/#the-config-tab):
...
You can replace existing Radius Clustering 3D segmenters by [configuring new ones in the UI](/platform/build/configure/services/vision/segmentation/#configure-an-obstacles_pointcloud-segmenter) or you can update the [Raw JSON configuration of your robots](/platform/build/configure/configuration/#the-config-tab):
...
You can replace existing Radius Clustering 3D segmenters by [configuring new ones in the UI](/platform/build/configure/services/vision/segmentation/#configure-a-detector_3d_segmenter) or you can update the [Raw JSON configuration of your robots](/platform/build/configure/configuration/#the-config-tab):
...
You must add and remove models using the [robot config](/platform/build/configure/configuration/).
...
You will need to first register the machine learning model file with the [ML model service](/platform/build/configure/services/ml/) and then add that registered model to a vision service.

## March 2023

### Added: Machine learning for image classification models

You can now [train](/platform/ml/train-model/) and [deploy](/platform/build/configure/services/ml/#create-an-ml-model-service) image classification models with the [data management service](/platform/build/configure/services/data/) and use your robot's image data directly within Viaml
Additionally, you can [upload and use](/platform/ml/upload-model/) existing machine learning models with your robots.
For more information on using data synced to the cloud to train machine learning models, read [Train a model](/platform/ml/train-model/).

### Added: Motion planning with new `constraint` parameter

A new parameter, [`constraint`](/platform/build/configure/services/motion/constraints/), has been added to the [Motion service API](/platform/build/configure/services/motion/#api), allowing you to define restrictions on the robot's movement.
...
When adding [frames](/platform/build/configure/services/frame-system/) to your robot's config in the Viam app, you can now use the **Frame System** subtab of the **Config** tab to more easily visualize the relative positions of frames.

### Added: Support for microcontrollers

Micro-RDK is a lightweight version of the RDK that can run on an ESP32.
Find more information in the [micro-RDK documentation](/platform/get-started/installation/prepare/microcontrollers/).

## January 2023

### Added: Remote control power input

On your robot's **Control** tab on the [Viam app](https://app.viam.com/), you can now set the power of a [base](/platform/build/configure/components/base/).
The base control UI previously always sent 100% power to the base's motors.

### Added: New encoder model: AMS AS5048

The [AMS AS5048](/platform/build/configure/components/encoder/ams-as5048/) is now supported.

### Added: GetLinearAcceleration method

The movement sensor API now includes a [GetLinearAcceleration](/platform/build/configure/components/movement-sensor/#getlinearacceleration) method.

### Added: Support for capsule geometry

The [motion service](/platform/build/configure/services/motion/) now supports capsule geometries.
...
You can now implement your own custom {{< glossary_tooltip term_id="resource" text="resources" >}} as [_modular resources_](/platform/registry/).
...
- [ADXL345](/platform/build/configure/components/movement-sensor/adxl345/): A 3-axis accelerometer
- [MPU-6050](/platform/build/configure/components/movement-sensor/mpu6050/): A 6-axis accelerometer and gyroscope
...
The [motion service](/platform/build/configure/services/motion/) is now agnostic to the networking topology of a robot.
...
For information on configuring any camera model, see [Camera Component](/platform/build/configure/components/camera/).
...
A new [servo model called `gpio`](/platform/build/configure/components/servo/gpio/) supports servos connected to non-Raspberry Pi boards.
...
Removed `width` and `height` from the response of the [`GetImage`](/platform/build/configure/components/camera/#getimage) method in the camera API.
