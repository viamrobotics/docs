...
<tbody style="vertical-align:top;">
    <tr>
        ...
        <td>Previously, the only way for users to implement custom resources was to use an SDK as a server. This meant that a user needed to run a viam-server for their custom component and add it to their main part as a remote. With custom modular resources, users no longer need to run separate server instances for each custom resource which saves additional network requests <br>
{{< alert title="Note" color="note" >}}
This breaking change affects ALL users who are using the Viam app to configure their robot. You will need to update to at latest version of the RDK version V3.0.0 to access your robot via the remote control page.
{{< /alert >}}
    </td>
    </tr>
<tbody>
...
<tbody style="vertical-align:top;">
    <tr>
        ...
        <stron>The default mime type for color cameras is now JPEG</strong>, which improves the streaming rate on a Raspberry Pi 4 to ~15 FPS across every SDK. </li>
