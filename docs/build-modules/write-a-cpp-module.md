...
[`viam-micro-server`](/reference/) works differently from the RDK (and `viam-server`), so creating modular resources for it is different from the process described on this page.
...
You will need to [recompile and flash your ESP32 yourself](/foundation/) instead of using Viam's prebuilt binary and installer.
{{< /alert >}}
...
For example, though you could technically implement a GPS as a sensor with just the `GetReadings` method, if you implement it as a movement sensor then you have access to methods like `GetCompassHeading` which allow you to use your GPS module with the [navigation service](/reference/apis/services/navigation/).
...
- If you are working with a component that doesn't fit into any of the existing component APIs, you can use the [generic component](/hardware/common-components/add-a-generic/) to build your own component API.
- If you are designing a service that doesn't fit into any of the existing service APIs, you can use the [generic service](/hardware/common-components/add-a-generic/) to build your own service API.
- It is also possible to [define an entirely new API](/reference/), but this is even more advanced than using `generic`.
...
For example, every model of motor built into Viam, as well as every custom model of motor provided by a module, all use the same API namespace triplet `rdk:component:motor` to indicate that they implement the [motor API](/hardware/common-components/add-a-motor/).
...
| [odrive](https://app.viam.com/module/viam/odrive) | [viamrobotics/odrive](https://github.com/viamrobotics/odrive) | Extends the built-in [motor API](/hardware/common-components/add-a-motor/) to support the ODrive motor. This module provides two models, one for a `canbus`-connected ODrive motor, and one for a `serial`-connected ODrive motor. |
...
<!-- | [module-example-cpp](https://app.viam.com/module/viam/module-example-cpp) | [viamrobotics/module-example-cpp](https://github.com/viamrobotics/module-example-cpp) | Extends the built-in [sensor API](/hardware/common-components/add-a-sensor/) to report wifi statistics. | -->
...
For example, if you wanted to add support for a new [base component](/hardware/common-components/add-a-base/) to Viam (the component that represents the central physical platform of your machine, to which all other components are connected), you would start by looking at the built-in `Base` component class, which is defined in the [Viam Python SDK](https://github.com/viamrobotics/viam-python-sdk) in the following file:
...
For example, if you wanted to add support for a new [base component](/hardware/common-components/add-a-base/) to Viam (the component that represents the central physical platform of your machine, to which all other components are connected), you would start by looking at the built-in `base` component package, which is defined in the [Viam Go SDK](https://github.com/viamrobotics/rdk/) in the following file:
...
For example, if you wanted to add support for a new [base component](/hardware/common-components/add-a-base/) to Viam (the component that represents the central physical platform of your machine, to which all other components are connected), you would start by looking at the built-in `Base` component class, which is defined in the [Viam C++ SDK](https://cpp.viam.dev/) in the following files:
...
{{% alert title="Note" color="note" %}}
In order to see resource-level debug logs when using your modular resource, you'll either need to run `viam-server` with the `-debug` option or [configure your machine or individual resource to display debug logs](/reference/).
{{% /alert %}}
...
1. Select the type of modular resource provided by your module, such as a [camera](/hardware/common-components/add-a-camera/), from the dropdown menu.
