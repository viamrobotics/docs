...
| **Cylinder** | `"cylinder"` | `r` (radius in mm), `l` (height in mm), optional `capped` (boolean, default `true`) | Solid cylindrical obstacles like cans, drums, and flat discs. Unlike capsule, a cylinder can be arbitrarily flat (no minimum height-to-radius ratio). Set `"capped": false` for an open tube: the end caps are omitted and the hollow interior is free space, so the planner can reach inside. |
