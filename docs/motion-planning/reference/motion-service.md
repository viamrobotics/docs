...
| `input_range_override`            | object | (none)  | Override a joint's position, velocity, or acceleration limits. The value is a map from frame name to a map from joint index (string) to `{"min": <value>, "max": <value>, "max_velocity": <value>, "max_acceleration": <value>}`. For example, `{"my-arm": {"3": {"min": 0, "max": 2}}}` restricts joint 3 of `my-arm` to the range 0-2. See the note below on how position limits differ from velocity and acceleration limits. |

### How input_range_override applies limits

Position limits and speed limits behave differently when overridden:

- **Position limits** (`min` and `max`) replace the kinematics file's values
  outright, with no clamping. An override can widen a joint's range past
  what the kinematics file declares, so verify your values against the
  arm manufacturer's documented limits before applying one.
- **Velocity and acceleration limits** (`max_velocity` and
  `max_acceleration`) can only be tightened. An override that specifies a
  value higher than the kinematics file's is ignored in favor of the lower
  value, so an override can slow a joint down but never speed it up. Omit
  either field to keep the model's own limit.

{{< alert title="Velocity and acceleration limits are not yet enforced" color="caution" >}}

The motion planner does not yet use `max_velocity` or `max_acceleration` to
constrain trajectories. Overriding them changes the kinematic model but has
no effect on planning or execution today. See
[Joint limits](/motion-planning/reference/kinematics/#joint-limits) for
details.
