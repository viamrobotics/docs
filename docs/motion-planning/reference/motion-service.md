...
| `input_range_override`            | object | (none)  | Override a joint's position, velocity, or acceleration limits. The value is a map from frame name to a map from joint index (string) to `{"min": <value>, "max": <value>, "max_velocity": <value>, "max_acceleration": <value>}`. Position limits (`min`/`max`) replace the kinematic limits outright. Velocity and acceleration limits can only be tightened (lowered), never raised past what the kinematics file declares, so an override can slow a joint down but cannot speed it up. Omit `max_velocity` or `max_acceleration` to keep the model's own limits. For example, `{"my-arm": {"3": {"min": 0, "max": 2}}}` restricts joint 3 of `my-arm` to the range 0-2. |
