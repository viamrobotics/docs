...
title: "Common errors and known issues"
weight: 45
layout: "docs"
type: "docs"
no_list: true
...
To view logs or get a remote shell on a machine see [Troubleshooting](/monitor/troubleshoot/).
...
- [Common installation errors](#common-installation-errors):
  - [The authenticity of host can't be established](#the-authenticity-of-host-hostnamelocal-cant-be-established)
  - [Host is down](#ssh-connect-to-host-hostnamelocal-port-22-host-is-down)
  - [Connection timed out](#ssh-connect-to-host-hostname-port-22-connection-timed-out)
  - [Something went wrong trying to read the squashfs image](#something-went-wrong-trying-to-read-the-squashfs-image)
  - [AppImages require FUSE to run](#appimages-require-fuse-to-run)
  - [PulseAudio: unable to connect](#pulseaudio-unable-to-connect-connection-refused)
- [Common connection errors](#common-connection-errors):
  - [Failed to connect; retrying](#failed-to-connect-retrying)
  - [Error connecting to peer, error not connected](#error-error-connecting-to-peer-error-not-connected)
  - [Cannot parse config: cannot unmarshal string](#error-cannot-parse-config-json-cannot-unmarshal-string-into-go-struct)
  - [Resource build error: unknown resource type](#error-resource-build-error-unknown-resource-type)
  - [Failed to connect to robot within time limit](#conn-time-out)
  - [Could not connect to machine part (context deadline exceeded)](#could-not-connect-to-machine-part-context-deadline-exceeded)
  - [Could not connect to machine part (ResourceExhausted)](#could-not-connect-to-machine-part-resourceexhausted)
  - [Error while dialing app](#error-while-dialing-app)
  - [Exceeded request limit on resource](#req-limit-exceeded)
- [Common module errors](#common-module-errors):
  - [Timed out waiting for module](#timed-out-waiting-for-module)
- [Common Micro-RDK errors](#common-micro-rdk-errors):
  - [Unable to properly process stun response](#unable-to-properly-process-stun-response-icestunencodingerror)
- [Common warnings](#common-warnings):
  - [Connection establishment failed](#connection-establishment-failed)
  - [Unable to create PeerConnection with module](#unable-to-create-peerconnection-with-module)
- [Other common errors](#other-common-errors):
  - [Accidental deletion of machines, locations, organizations, or accounts](#accidental-deletion-of-machines-locations-organizations-or-accounts)
...
**Description:** When following our [installation guides](/set-up-a-machine/first-machine/), you will likely encounter this message the first time you try to make an `ssh` connection to your newly-imaged {{< glossary_tooltip term_id="board" text="board" >}}.
...
- If that fails, try re-imaging your board following the [installation guide](/set-up-a-machine/first-machine/) appropriate for your board.
...
**Description:** The `viam-server` [installation](/set-up-a-machine/first-machine/) or [update](/reference/viam-server/) process may have been interrupted partway, with some files either partially-written or missing.

**Solution:** Reinstall `viam-server` following the [installation instructions](/set-up-a-machine/first-machine/).
...
- Is the {{< glossary_tooltip term_id="board" text="board" >}} connected to the internet?
- Is `viam-server` running on the machine?
- Are there errors in the machine's logs?

To check whether `viam-server` is running and to read its logs, follow the steps in [Machine shows as offline](/monitor/troubleshoot/#machine-shows-as-offline).
...
**Solution:** See [Network Troubleshooting](#network-troubleshooting) section.
...
To adjust the per-resource limit for modules, you can set the `VIAM_RESOURCE_REQUESTS_LIMIT` [environment variable on your machine](/reference/viam-server/#environment-variables) to a positive integer higher than the default, 100.
...
**Solution:** To adjust the initial app connection timeout, you can set the `VIAM_CONFIG_READ_TIMEOUT` [environment variable on your machine](/reference/viam-server/#environment-variables) to a positive integer higher than the default, 15.
...
- If you are the module author, consider packaging the module with required dependencies so they don't need to be downloaded on startup.
  For Python modules, you can package your module with dependencies by using the PyInstaller steps when [uploading your module](/build-modules/deploy-a-module/#release-with-cloud-build).
...
- If a Go module hangs rather than timing out, see [Advanced debugging for Go modules](/monitor/troubleshoot/#advanced-debugging-for-go-modules) to capture a stack trace.
