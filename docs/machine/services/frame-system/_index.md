...
no_list: true
icon: true
images: ["/machine/services/icons/frame-system.svg"]
tags: ["frame system", "services"]
aliases:
  - /machine/services/frame-system/
...
![Visualization of a wheeled base configured with motors and a mounted camera in the frame system tab of the Viam app UI](/machine/services/frame-system/frame_system_wheeled_base.png)

The frame system is the basis for some of Viam's other services, like [motion](/machine/services/motion/) and [vision](/services/vision/).
...
2. From the left-hand menu, select your component.
   ...
   {{<imgproc src="/machine/services/frame-system/frame_card.png" resize="300x" style="max-width: 500px" alt="Frame card for a camera with the default reference frame settings">}}
...
For [base components](/machine/components/base/), Viam considers `+X` to be to the right, `+Y` to be forwards, and `+Z` to be up.
...
- [A Reference Frame:](/machine/services/frame-system/frame-config/) A component attached to a static surface
- [Nested Reference Frames:](/machine/services/frame-system/nested-frame-config/) A component attached to another, dynamic, component
...
![Default frame system configuration grid visualization for a single component, shown in the Frame System Editor](/machine/services/frame-system/frame_system_basic.png)
...
Consider a machine configured with a [`jetson` board](/machine/components/board/), wired to a [`webcam` camera](/machine/components/camera/webcam/) and a [`wheeled` base](/machine/components/base/wheeled/) with two [motors](/machine/components/motor/) driving its wheels.
...
![Example machine's default frame configuration shown in the Frame System Editor. All components are stuck on top of each other](/machine/services/frame-system/demo_base_unedited.png)
...
![Base translated 200mm forwards shown in the Frame System Editor](/machine/services/frame-system/demo_base_edited.png)
...
![Camera translated 50 mm overhead shown in the Frame System Editor](/machine/services/frame-system/demo_camera_edited_1.png)
...
![Camera oriented .5 degrees OX shown in the Frame System Editor](/machine/services/frame-system/demo_camera_edited_2.png)

Now that the frame system is accurately configured with the machine's spatial orientation, [motion service](/machine/services/motion/) methods that take in reference frame information can be utilized.

### Display options

Click and drag on the **Frame** visualization to view the display from different angles, and pinch to zoom in and out:

{{<gif webm_src="/machine/services/frame-system/frame_system_demo.webm" mp4_src="/machine/services/frame-system/frame_system_demo.mp4" alt="The frame system visualization zooming in and out around the example robot with a camera, board, and wheeled base.">}}
...
![Default Perspective Camera view shown in the Frame System Editor](/machine/services/frame-system/demo_perspective.png)

{{% /tab %}}
{{% tab name="Orthographic" %}}

![Orthographic Camera view shown in the Frame System Editor](/machine/services/frame-system/demo_orthographic.png)
...
![Demo robot with default box bounds added to the wheeled base, shown in the Frame System Editor](/machine/services/frame-system/demo_bound_box.png)

For example:

![Demo robot with translated box bounds added to the wheeled base, shown in the Frame System Editor](/machine/services/frame-system/demo_bound_box_translation.png)

{{< /tab >}}
{{% tab name="Sphere" %}}

![Demo robot with default sphere bounds added to the wheeled base, shown in the Frame System Editor](/machine/services/frame-system/demo_bound_sphere.png)

{{% /tab %}}
{{% tab name="Capsule" %}}

![Demo robot with default capsule bounds added to the wheeled base, shown in the Frame System Editor](/machine/services/frame-system/demo_bound_capsule.png)
...
![an example of a logged frame system](/machine/services/frame-system/frame_sys_log_example.png)

Consider the example of nested reference frame configuration where [two dynamic components are attached](/machine/services/frame-system/nested-frame-config/): a robotic arm, `A`, attaches to a gantry, `G`, which in turn is fixed in place at a point on the `World` of a table.

The resulting tree of reference frames looks like:

![reference frame tree](/machine/services/frame-system/frame_tree.png)
...
For more information, see the [Go SDK Docs](https://pkg.go.dev/go.viam.com/rdk/machine/components/arm#Arm).
...
- In our [example of nested dynamic attachment](/machine/services/frame-system/nested-frame-config/), the arm can be managed by the frame system without additional transforms because the base of the arm is fixed with respect to the gantry's platform, and the gantry's origin is fixed with respect to the `world` reference frame (centered at `(0, 0, 0)` in the machine's coordinate system).

  However, an arm with an attached [camera](/machine/components/camera/) might generate additional information about the poses of other objects with respect to references frames on the machine.
  ...
  If a [camera](/machine/components/camera/) is looking for an apple or an orange, the arm can be commanded to move to the detected fruit's location by providing an additional transform that contains the detected pose with respect to the camera that performed the detection.
...
- You can pass a detected object's frame information to the `supplemental_transforms` parameter in your calls to Viam's motion service's [`GetPose`](/machine/services/motion/#getpose) method.
