...
icon: true
images: ["/machine/services/icons/motion.svg"]
no_list: true
aliases:
  - "/machine/services/motion/"
# SME: Motion team
---
...
{{< relatedcard link="/machine/components/base/" >}}
{{< relatedcard link="/machine/components/arm/" >}}
{{< relatedcard link="/machine/components/gripper/" >}}
{{< /cards >}}
...
For more information, see the [Python SDK Docs](https://python.viam.dev/autoapi/viam/machine/services/motion/index.html#viam.services.motion.MotionClient.move).
...
For more information, see the [Go SDK Docs](https://pkg.go.dev/go.viam.com/rdk/machine/services/motion#Service).
...
For more information, see the [Python SDK Docs](https://python.viam.dev/autoapi/viam/machine/services/motion/index.html#viam.services.motion.MotionClient.get_pose).

The following code example gets the pose of the tip of a [gripper](/machine/components/gripper/) named `my_gripper` which is attached to the end of an arm, in the "world" `reference_frame`:
...
Move a [base](/machine/components/base/) component to a destination [`Pose`](https://python.viam.dev/autoapi/viam/proto/common/index.html#viam.proto.common.Pose) on a {{< glossary_tooltip term_id="slam" text="SLAM" >}} map.
...
`MoveOnMap()` is intended for use with the [navigation service](/machine/services/navigation/), providing autonomous indoor navigation for rover [bases](/machine/components/base/).

{{< alert title="Requirements" color="info" >}}
To use `MoveOnMap()`, your [SLAM service](/machine/services/slam/) must implement `GetPointCloudMap()` and `GetPosition()`

Make sure the [SLAM service](/machine/services/slam/) you use alongside the this motion service supports the following methods in its {{< glossary_tooltip term_id="model" text="model's" >}} implementation of the [SLAM service API](/machine/services/slam/#api):
...
- `slam_service_name` ([ResourceName](https://python.viam.dev/autoapi/viam/proto/common/index.html#viam.proto.common.ResourceName)): The `ResourceName` of the [SLAM service](/machine/services/slam/) from which the SLAM map is requested.
- `configuration` [(Optional[MotionConfiguration])](https://python.viam.dev/autoapi/viam/proto/service/motion/index.html#viam.proto.service.motion.MotionConfiguration): The configuration you want to set across this machine for this motion service. This parameter and each of its fields are optional.
  - `obstacle_detectors` [(Iterable[ObstacleDetector])](https://python.viam.dev/autoapi/viam/proto/service/motion/index.html#viam.proto.service.motion.ObstacleDetector): The names of each [vision service](/services/vision/) and [camera](/machine/components/camera/) resource pair you want to use for transient obstacle avoidance.
...
For more information, see the [Python SDK Docs](https://python.viam.dev/autoapi/viam/machine/services/motion/index.html#viam.services.motion.MotionClient.move_on_map).
...
- `ctx` [(Context)](https://pkg.go.dev/context): A Context carries a deadline, a cancellation signal, and other values across API boundaries.
- `req` [MoveOnMapReq](https://pkg.go.dev/go.viam.com/rdk/machine/services/motion#MoveOnMapReq): A `MoveOnMapReq` which contains the following values:
  ...
  - `SlamName` [(resource.Name)](https://pkg.go.dev/go.viam.com/rdk/resource#Name): The `resource.Name` of the [SLAM service](/machine/services/slam/) from which the SLAM map is requested.
  - `MotionConfig` [(\*MotionConfiguration)](https://pkg.go.dev/go.viam.com/rdk/machine/services/motion#MotionConfiguration): The configuration you want to set across this machine for this motion service. This parameter and each of its fields are optional.
    - `ObstacleDetectors` [([]ObstacleDetectorName)](https://pkg.go.dev/go.viam.com/rdk/machine/services/motion#ObstacleDetectorName): The names of each [vision service](/services/vision/) and [camera](/machine/components/camera/) resource pair you want to use for transient obstacle avoidance.
...
- [(ExecutionID)](https://pkg.go.dev/go.viam.com/rdk/machine/services/motion#ExecutionID): ExecutionID of the `MoveOnMap` call.
- [(error)](https://pkg.go.dev/builtin#error): An error, if one occurred.
...
Move a [base](/machine/components/base/) component to a destination GPS point, represented in geographic notation _(latitude, longitude)_.
Use a [movement sensor](/machine/components/movement-sensor/) to check the location of the machine.
...
`MoveOnGlobe()` is intended for use with the [navigation service](/machine/services/navigation/), providing autonomous GPS navigation for rover [bases](/machine/components/base/).

{{< alert title="Requirements" color="info" >}}
To use `MoveOnGlobe()`, your movement sensor must be able to measure the GPS location and orientation of the machine.

Make sure the [movement sensor](/machine/components/movement-sensor/) you use supports usage of the following methods in its {{< glossary_tooltip term_id="model" text="model's" >}} implementation of the [movement sensor API](/machine/components/movement-sensor/#api).
...
Specifying `heading` in a request to `MoveOnGlobe` is not currently recommended if the minimum turning radius of your component is greater than zero, as this combination may cause high latency in the [motion planning algorithms](/machine/services/motion/algorithms/).
...
Translation in obstacles is not supported by the [navigation service](/machine/services/navigation/).
...
- `component_name` [(ResourceName)](https://python.viam.dev/autoapi/viam/proto/common/index.html#viam.proto.common.ResourceName): The `ResourceName` of the base to move.
- `destination` [(GeoPoint)](https://python.viam.dev/autoapi/viam/machine/components/movement_sensor/index.html#viam.components.movement_sensor.GeoPoint): The location of the component's destination, represented in geographic notation as a [GeoPoint](https://python.viam.dev/autoapi/viam/machine/components/movement_sensor/index.html#viam.components.movement_sensor.GeoPoint) _(lat, lng)_.
- `movement_sensor_name` [(ResourceName)](https://python.viam.dev/autoapi/viam/proto/common/index.html#viam.proto.common.ResourceName): The `ResourceName` of the [movement sensor](/machine/components/movement-sensor/) that you want to use to check the machine's location.
...
For more information, see the [Python SDK Docs](https://python.viam.dev/autoapi/viam/machine/services/motion/index.html#viam.services.motion.MotionClient.move_on_globe).
...
- `ctx` [(Context)](https://pkg.go.dev/context): A Context carries a deadline, a cancellation signal, and other values across API boundaries.
- `req` [MoveOnGlobeReq](https://pkg.go.dev/go.viam.com/rdk/machine/services/motion#MoveOnGlobeReq): A `MoveOnGlobeReq` which contains the following values:
  - `componentName` [(resource.Name)](https://pkg.go.dev/go.viam.com/rdk/resource#Name): The `resource.Name` of the base to move.
  ...
  - `movementSensorName` [(resource.Name)](https://pkg.go.dev/go.viam.com/rdk/resource#Name): The `resource.Name` of the [movement sensor](/machine/components/movement-sensor/) that you want to use to check the machine's location.
...
- [(ExecutionID)](https://pkg.go.dev/go.viam.com/rdk/machine/services/motion#ExecutionID): ExecutionID of the `MoveOnGlobe` call.
- [(error)](https://pkg.go.dev/builtin#error): An error, if one occurred.
...
For more information, see the [Python SDK Docs](https://python.viam.dev/autoapi/viam/machine/services/motion/index.html#viam.services.motion.MotionClient.stop_plan).
...
- `ctx` [(Context)](https://pkg.go.dev/context): A Context carries a deadline, a cancellation signal, and other values across API boundaries.
- `req` [StopPlanReq](https://pkg.go.dev/go.viam.com/rdk/machine/services/motion#StopPlanReq): A `StopPlanReq` which contains the following values:
...
For more information, see the [Python SDK Docs](https://python.viam.dev/autoapi/viam/machine/services/motion/index.html#viam.services.motion.MotionClient.get_plan).
...
- `ctx` [(Context)](https://pkg.go.dev/context): A Context carries a deadline, a cancellation signal, and other values across API boundaries.
- `req` [PlanHistoryReq](https://pkg.go.dev/go.viam.com/rdk/machine/services/motion#PlanHistoryReq): A `PlanHistoryReq` which contains the following values:
  - `componentName` [(resource.Name)](https://pkg.go.dev/go.viam.com/rdk/resource#Name): The `resource.Name` of the base to stop.
  ...
  - `executionID` [(ExecutionID)](https://pkg.go.dev/go.viam.com/rdk/machine/services/motion#PlanHistoryReq): If non empty, the response will return the plans of the provided execution & component. Useful for retrieving plans from executions before the current execution.
  - `extra` [(map\[string\]interface{})](https://go.dev/blog/maps): Extra options to pass to the underlying RPC call.

**Returns:**

- [([]PlanWithStatus)](https://pkg.go.dev/go.viam.com/rdk/machine/services/motion#PlanWithStatus): PlanWithStatus contains a plan, its current status, and all state changes that came prior sorted by ascending timestamp.
- [(error)](https://pkg.go.dev/builtin#error): An error, if one occurred.
...
Returns the statuses of plans created by [`MoveOnGlobe`](/machine/services/motion/#moveonglobe) or [`MoveOnMap`](/machine/services/motion/#moveonmap) calls that meet at least one of the following conditions since the motion service initialized:
...
- [(ListPlanStatusesResponse)](https://python.viam.dev/autoapi/viam/machine/services/motion/index.html#viam.services.motion.ListPlanStatusesResponse)
...
For more information, see the [Python SDK Docs](https://python.viam.dev/autoapi/viam/machine/services/motion/index.html#viam.services.motion.MotionClient.list_plan_statuses).
...
- `ctx` [(Context)](https://pkg.go.dev/context): A Context carries a deadline, a cancellation signal, and other values across API boundaries.
- `req` [ListPlanStatusesReq](https://pkg.go.dev/go.viam.com/rdk/machine/services/motion#ListPlanStatusesReq): A `ListPlanStatusesReq` which contains the following values:
  - `onlyActivePlans` [(bool)](https://pkg.go.dev/builtin#bool): If `true`, the response will only return plans which are executing.
  - `extra` [(map\[string\]interface{})](https://go.dev/blog/maps): Extra options to pass to the underlying RPC call.

**Returns:**

- [([]PlanStatusWithID)](https://pkg.go.dev/go.viam.com/rdk/machine/services/motion#PlanStatusWithID): `PlanStatusWithID` describes the state of a given plan at a point in time plus the `PlanId`, `ComponentName` and `ExecutionID` the status is associated with.
- [(error)](https://pkg.go.dev/builtin#error): An error, if one occurred.
...
For more information, see the [Python SDK Docs](https://python.viam.dev/autoapi/viam/machine/services/motion/client/index.html#viam.services.motion.client.MotionClient.do_command).
...
For more information, see the [Python SDK Docs](https://python.viam.dev/autoapi/viam/machine/services/motion/client/index.html#viam.services.motion.client.MotionClient.close).
...
You can test motion on your machine from the [**CONTROL** tab](/app/fleet/control/).

![Motion card on the Control tab](/machine/services/motion/motion-rc-card.png)
