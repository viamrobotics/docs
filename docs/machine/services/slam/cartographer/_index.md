...
tags: ["slam", "services"]
icon: true
images: ["/machine/services/icons/slam.svg"]
aliases:
  - "/machine/services/slam/run-slam-cartographer/"
  - "/machine/services/slam/cartographer/"
# SMEs: Kat, Jeremy
---
...
See the [Use Modules](/app/registry/#use-modules) section for instructions on using a module from the Viam registry on your machine.
...
- You must have an RPlidar, such as the [RPlidar A1](https://www.slamtec.com/en/Lidar/A1) or [RPlidar A3](https://www.slamtec.com/en/Lidar/A3), physically connected to your machine.
  ...
  Furthermore, ensure that the center of the RPlidar is mounted at the center of your machine's [base](/machine/components/base/).
...
- Add only inertial measurement unit (IMU) data
  - Requires a movement sensor that supports [`AngularVelocity`](/machine/services/navigation/#angular-velocity) and [`LinearAcceleration`](/machine/services/navigation/#linear-acceleration) readings
- Add only odometry data
  - Requires a movement sensor that collects [`Position`](/machine/services/navigation/#position) and [`Orientation`](/machine/services/navigation/#orientation) data (for example, [`wheeled-odometry`](/machine/components/movement-sensor/wheeled-odometry/))
- Add both IMU _and_ odometry data
  - Requires all four of the above kinds of data, merged together using the [`merged` movement sensor model](/machine/components/movement-sensor/merged/)
...
1. Add the data management service to your machine:
   ...
   See the [data management service](/app/data/) for more information.
...
2. Enable data capture for your camera, and for your movement sensor if you would like to use IMU data, odometry data, or both:
   ...
   - For the required LiDAR camera, choose the `NextPointCloud` method.
     ...
     {{<imgproc src="/machine/services/slam/rplidar-capture.png" resize="x1100" declaredimensions=true alt="An R P lidar camera configured in the Viam app config builder with next point cloud configured for capture at 5 hertz." >}}
...
{{<imgproc src="/machine/services/slam/imu-capture.png" resize="x1100" declaredimensions=true alt="An IMU configured in the Viam app config builder with angular velocity and linear acceleration both configured for capture at 20 hertz." >}}
...
{{<imgproc src="/machine/services/slam/odometer-capture.png" resize="x1100" declaredimensions=true alt="A wheeled odometer configured in the Viam app config builder with position and orientation both configured for capture at 20 hertz." >}}
...
{{<imgproc src="/machine/services/slam/merged-capture.png" resize="x1100" declaredimensions=true alt="An IMU configured in the Viam app config builder with angular velocity, linear acceleration, position, and orientation all configured for capture at 20 hertz." >}}

{{% /tab %}}
{{< /tabs >}}

{{< alert title="Tip" color="tip" >}}
Note that [Data Capture](/app/data/capture/) continuously monitors and captures your machine’s sensor data while the machine is running. To avoid incurring charges while not in use, [turn off data capture for your sensors](/app/data/capture/) once you have finished your SLAM session.
{{< /alert >}}
...
- configures the `viam:slam:cartographer` service and the [data management service](/app/data/)
...
1. Start a mapping session:
   ...
   ![slam RC card start session](/machine/services/slam/slam-RC-card-start-session.png)

   While the slam session is starting, you will see a loading screen.

   ![slam RC card wait for session to finish starting](/machine/services/slam/slam-RC-card-wait-for-session-to-finish-starting.png)

   Once the slam session has finished starting, your first pointcloud will appear.

   ![slam RC card first pointcloud](/machine/services/slam/slam-RC-card-first-pointcloud.png)

   You can see that your cloud slam session is in progress from your **Location** page's **SLAM library** tab.

   ![offline mapping maps computing table](/machine/services/slam/offline-mapping-maps-computing-table.png)
   ...
   ![offline mapping available maps](/machine/services/slam/offline-mapping-available-maps.png)

   You can click `View map` to view the map in a dynamic pointcloud viewer.

   ![slam library view map](/machine/services/slam/slam-library-view-map.png)
...
1.  Start a mapping session:
    ...
    ![slam RC card localize only](/machine/services/slam/slam-RC-card-localize-only.png)
...
{{<imgproc src="/machine/services/slam/offline-mapping-pointcloud-data.png" resize="1200x" declaredimensions=true alt="UI showing captured point clouds">}}

Example of previously captured IMU data:

{{<imgproc src="/machine/services/slam/offline-mapping-imu-data.png" resize="1200x" declaredimensions=true alt="UI showing captured sensor data">}}
...
{{<imgproc src="/machine/services/slam/offline-mapping-generate-map.png" resize="1200x" declaredimensions=true alt="UI for creating a new map from captured data">}}
