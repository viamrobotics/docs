...
images: ["/machine/services/vision/dog-detector.png"]
aliases:
...
You can [add an existing model](/ml/upload-model/) or [train your own models](/app/ml/train-model/) for object detection and classification using data from the [data management service](/app/data/).
...
If you [trained your ML model using the Viam app](/app/ml/train-model/), your `mlmodel` tensors are already named in this fashion, and you can proceed to [test your detector or classifier](#test-your-detector-or-classifier).
...
{{<gif webm_src="/machine/services/vision/mug-classifier.webm" mp4_src="/machine/services/vision/mug-classifier.mp4" alt="A classification model run against an image containing a mug." max-width="250px" class="alignright">}}

If you have images stored in the [Viam cloud](/app/data/), you can run your classifier against your images in the [Viam app](https://app.viam.com/).

1. Navigate to the [Data tab](/app/data/view/) and click on the **Images** subtab.
...
If you intend to use the detector or classifier with a camera that is part of your machine, you can test your detector or classifier from the [**Control tab**](/app/fleet/control/) or with code:

1. Configure a [camera component](/machine/components/camera/).

   {{< alert title="Tip" color="tip" >}}
   This is the camera whose name you need to pass to vision service methods.
   {{< /alert >}}

2. (Optional) If you would like to see detections or classifications from the **Control tab**, configure a [transform camera](/machine/components/camera/transform/) with the following attributes:
...
4. Navigate to the **CONTROL** tab, click on your transform camera and toggle it on.
   ...
   ![Viam app control tab interface showing bounding boxes around two office chairs, both labeled "chair" with confidence score "0.50."](/machine/services/vision/chair-detector.png)

   If you've configured a classifier, the transform camera will now show classifications on the image.

   ![Model recognizes a star on camera feed](/machine/services/model-on-camera.png)
...
To learn more about how to use detection, see the [Python SDK docs](https://python.viam.dev/autoapi/viam/machine/services/vision/index.html).
...
To learn more about how to use classification, see the [Python SDK docs](https://python.viam.dev/autoapi/viam/machine/services/vision/index.html).
