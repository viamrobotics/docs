...
aliases:
  - "/machine/components/encoder/single/"
# SMEs: Rand
---

A `single` encoder sends a signal from the rotating encoder over a single wire to one pin on the [board](/machine/components/board/).
The direction of spin is dictated by the [motor](/machine/components/motor/) that has this encoder's name in its `encoder` attribute field.
...
![Configuration of a single encoder in the Viam app config builder.](/machine/components/encoder/configure-single.png)
...
| ---- | ---- | --------- | ----------- |
| `board` | string | **Required** | The `name` of the [board](/machine/components/board/) to which the encoder is wired. |
