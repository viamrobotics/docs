...
aliases:
  - "/machine/components/base/boat/"
# SMEs: Eliot
---

A `boat` base is a model for a mobile robotic boat.
To configure a `boat` base as a component of your machine, first configure the [board](/machine/components/board/) controlling the base and any [motors](/machine/components/motor/) attached to the base.
...
| `IMU` | string | **Required** | Name of the [Inertial Measurement Unit](/machine/components/movement-sensor/#imu-configuration) in the boat. |
| `Motors` | string[] | **Required** | JSON struct containing the configuration attributes for each motor attached to the boat. |

Each [motor](/machine/components/motor/) inside of `Motors` has the following attributes available:
