...
aliases:
  - "/machine/components/base/sensor-controlled/"
...
In order to use feedback control, you must provide a movement sensor that implements [AngularVelocity()](/machine/components/movement-sensor/#getangularvelocity) and [LinearVelocity()](/machine/components/movement-sensor/#getlinearvelocity). This will enable feedback control for [SetVelocity()](/machine/components/base/#setvelocity).

In order to use feedback control for [Spin()](/machine/components/base/#spin), you must also provide a movement sensor that implements [Orientation()](/machine/components/movement-sensor/#getorientation).

In order to use feedback control for [MoveStraight()](/machine/components/base/#spin), you must also provide a movement sensor that implements [Position()](/machine/components/movement-sensor/#getposition). Additionally, heading feedback control while moving straight can be used by providing a movement sensor that implements [Orientation()](/machine/components/movement-sensor/#getorientation) or [CompassHeading()](/machine/components/movement-sensor/#getcompassheading).
{{% /alert %}}

To configure a `sensor-controlled` base as a component of your machine, first configure the [model of base](/machine/components/base/) you want to wrap with feedback control and each required [movement sensor](/machine/components/movement-sensor/).
To see what models of movement sensor report which feedback, reference the appropriate column in [Movement Sensor API](/machine/components/movement-sensor/#api).
...
{{< imgproc src="/machine/components/base/sensor-controlled-base-ui-config.png" alt="An example configuration for a sensor-controlled base in the Viam app config builder" resize="1200x" style="width: 600x">}}
...
- [SetVelocity()](/machine/components/base/#setvelocity): available if base is configured to receive angular and linear velocity feedback.
- [Spin()](/machine/components/base/#spin): available if base is configured to receive orientation feedback.
- [MoveStraight()](/machine/components/base/#movestraight): available if base is configured to receive position feedback.

For example, a [Viam Rover](/get-started/try-viam/rover-resources/) using `sensor-controlled` base following both an [angular](/machine/components/base/#spin) and [linear](/machine/components/base/#movestraight) velocity command:

{{<gif webm_src="/machine/components/encoded-motor/base_moving.webm" mp4_src="/machine/components/encoded-motor/base-moving.mp4" alt="A Viam rover turning in a half circle" max-width="400px" >}}

The position, orientation, and linear and angular velocity of the rover changing as it moves, as measured by a [movement sensor](/machine/components/movement-sensor/):

{{<gif webm_src="/machine/components/encoded-motor/controls_change.webm" mp4_src="/machine/components/encoded-motor/controls_change.mp4" alt="The control tab of a movement sensor on a base with encoded motors as it turns">}}
