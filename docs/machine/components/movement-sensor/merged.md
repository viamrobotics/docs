...
aliases:
  - "/machine/components/movement-sensor/merged/"
# SMEs: Rand
---
...
This is especially useful if you want to get readings of position and orientation _or_ linear and angular velocity at the same time, which are normally separately supported and returned by [`GPS`](/machine/components/movement-sensor/#supported-models) or [`IMU`](/machine/components/movement-sensor/#supported-models) models, respectively.

To reduce velocity error when your machine is using the [navigation service](/machine/services/navigation/), aggregate `Position()` from a [`GPS`](/machine/components/movement-sensor/#supported-models) and `Orientation()` from an [`IMU`](/machine/components/movement-sensor/#supported-models) movement sensor in a `merged` model.

Configure a [navigation service](/machine/services/navigation/) to use your merged sensor to navigate.

Before configuring a `merged` movement sensor, configure each movement sensor you want to merge as an individual component according to its [model's configuration instructions](/machine/components/movement-sensor/#supported-models).
...
{{< imgproc src="/machine/components/movement-sensor/merged-config-builder.png" alt="Creation of an `merged` movement sensor in the Viam app config builder." resize="1200x" style="width:650px" >}}

Fill in the attributes as applicable to your movement sensor, according to the table below.
For example:

{{< imgproc src="/machine/components/movement-sensor/merged-config-builder-attributes.png" alt="Creation of an `merged` movement sensor with attributes filled in in the Viam app config builder." resize="1200x" style="width:650px" >}}
...
- Get the properties supported by each model from its [model configuration documentation](/machine/components/movement-sensor/#supported-models), or by calling [`GetProperties()`](/machine/components/movement-sensor/#getproperties) on the sensor.
