...
aliases:
  - "/machine/components/motor/dmc4000/"
# SMEs: Rand, James
---
...
Whereas a basic low-level stepper driver supported by the [`gpiostepper` model](/machine/components/motor/gpiostepper/) sends power to a stepper motor based on PWM signals from GPIO pins, the DMC40x0 motion controller has many motion control features.
When using it, you do not need to configure a [board component](/machine/components/board/) because it handles computation and signal creation on the motion controller itself.
...
![A DMC4000 motor config with the attributes configured per the JSON on the next tab in this doc.](/machine/components/motor/dmc4000-config-ui.png)
...
For more information on `do_command`, see the [Python SDK Docs](https://python.viam.dev/autoapi/viam/machine/components/component_base/index.html#viam.components.component_base.ComponentBase.do_command).
...
For more information, see the Go SDK docs on [`Home`](https://pkg.go.dev/go.viam.com/rdk/machine/components/motor/dmc4000#Motor.Home) and on [`DoCommand`](https://pkg.go.dev/go.viam.com/rdk/machine/components/motor/dmc4000#Motor.DoCommand).
...
For more information, see the Go SDK Docs on [`Jog`](https://pkg.go.dev/go.viam.com/rdk/machine/components/motor/dmc4000#Motor.Jog) and on [`DoCommand`](https://pkg.go.dev/go.viam.com/rdk/machine/components/motor/dmc4000#Motor.DoCommand).
...
For more information, see the Go SDK Docs on [`Raw`](https://pkg.go.dev/go.viam.com/rdk/machine/components/motor/dmc4000#Motor.Raw) and on [`DoCommand`](https://pkg.go.dev/go.viam.com/rdk/machine/components/motor/dmc4000#Motor.DoCommand).
