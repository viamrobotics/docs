...
aliases:
  - "/machine/components/motor/gpiostepper/"
# SMEs: Rand, James
---
...
To configure a `gpiostepper` motor as a component of your machine, first configure the [board](/machine/components/board/) to which the motor driver is wired.
Then, add the motor:
...
![Screenshot of a gpiostepper motor config with the step and dir pins configured to pins 13 and 15.](/machine/components/motor/gpiostepper-config-ui.png)
...
![motor-gpiostepper-json.](/machine/components/motor/motor-gpiostepper-json.png)
...
| ---- | ---- | --------- | ---------- |
| `board` | string | **Required** | `name` of the [board](/machine/components/board/) the motor driver is wired to. |
| `pins` | object | **Required** |  A struct containing the [board](/machine/components/board/) {{< glossary_tooltip term_id="pin-number" text="pin numbers" >}} that the `step` and `dir` pins of the motor driver are wired to. |
