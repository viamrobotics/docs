...
aliases:
  - "/machine/components/motor/28byj48/"
# SMEs: Rand, James
---
...
To configure a `28byj48` motor as a component of your machine, first configure the [board](/machine/components/board/) to which the motor driver is wired.
Then, add the motor:
...
![A 28byj48 motor config.](/machine/components/motor/28byj48-config-ui.png)
...
| `pins` | object | **Required** | A JSON object containing the {{< glossary_tooltip term_id="pin-number" text="pin numbers" >}} the `in1`, `in2`, `in3`, and `in4` pins of the motor driver are wired to on the [board](/machine/components/board/). |
