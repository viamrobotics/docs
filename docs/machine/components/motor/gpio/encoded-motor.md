...
aliases:
  - "/machine/components/motor/gpio/encoded-motor/"
# SMEs: Rand, James
---

Use an [encoder](/machine/components/encoder/) with a motor to create a closed feedback loop for better control of your machine.
...
See the [encoder component documentation](/machine/components/encoder/) for more information on encoders.

Viam supports `gpio` model motors with encoders.
To configure an encoded motor, you must configure the encoder [per the encoder documentation](/machine/components/encoder/) and then configure a `gpio` motor with an `encoder` attribute in addition to the [standard `gpio` model attributes](/machine/components/motor/gpio/).
...
![An encoded motor config in the Viam app UI.](/machine/components/motor/encoded-config-ui.png)
...
![Same example JSON as on the JSON example tab, with notes alongside it. See attribute table below for all the same information.](/machine/components/motor/motor-encoded-dc-json.png)

{{% /tab %}}
{{< /tabs >}}

In addition to the [attributes for a non-encoded motor](/machine/components/motor/gpio/), the following attributes are available for encoded DC motors:
...
The attribute [`max_rpm`](/machine/components/motor/gpio/) is not required or available for encoded `gpio` motors.

{{% /alert %}}

{{% alert title="Important" color="note" %}}

If `encoder` is model [`AM5-AS5048`](/machine/components/encoder/ams-as5048/),`ticks_per_rotation` must be `1`, as `AM5-AS5048` is an absolute encoder which provides angular measurements directly.
...
![Example wiring diagram with a Raspberry Pi, brushed DC motor, 12V power supply, and Pololu MAX14870 motor driver. The DIR pin of the driver is wired to pin 18 on the Pi. PWM goes to pin 16. The motor's encoder signal wires (out a and out b) go to pins 11 and 13 on the Pi. The motor's main power wires are connected to the motor driver while its encoder logic power wires are connected to the Pi.](/machine/components/motor/motor-encoded-dc-wiring.png)

## Test the motor

Once your motor is configured and connected, go to the [**CONTROL** tab](/app/fleet/control/) and click on the motor's dropdown panel.
...
For example, a [Viam Rover](/get-started/try-viam/rover-resources/) with encoded motors following both an [angular](/machine/components/base/#spin) and [linear](/machine/components/base/#movestraight) velocity command:
