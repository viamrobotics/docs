...
aliases:
  - "/machine/components/gantry/oneaxis/"
  - "/machine/components/gantry/single-axis/"
# SME: Rand, Martha
---

Configure a `single-axis` gantry to integrate a single-axis gantry into your machine.
Before configuring the gantry, configure any [motor components](/machine/components/motor/) that are part of the gantry.
...
![Creation of a single-axis gantry component in the Viam app config builder.](/machine/components/gantry/single-axis-ui-config.png)
...
| `motor` | string | **Required** | `name` of the [motor](/machine/components/motor/) that moves the gantry's actuator. |
| `board`  |  string | Optional | `name` of the [board](/machine/components/board/) containing the limit switches and pins. If `limit_pins` exist, `board` is required. |
| `limit_pins`  | object | Optional | The `boards`'s pins attached to the limit switches on either end. If the [motor](/machine/components/motor/) used does not include an [encoder](/machine/components/motor/gpio/encoded-motor/), `limit_pins` are required to be set. |
