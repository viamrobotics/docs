...
You can browse existing modules in the [Viam Registry](/platform/registry/#the-viam-registry) to find one that supports your custom hardware or software, or you can write your own module.

Modules run alongside `viam-server` as separate processes, communicating with `viam-server` over UNIX sockets.
When a module initializes, it registers its {{< glossary_tooltip term_id="model" text="model or models" >}} and the associated [APIs](/platform/build/program/apis/) with `viam-server`, making the new model available for use.

In most cases, the {{< glossary_tooltip term_id="modular-resource" text="modular resources" >}} you create should be a new {{< glossary_tooltip term_id="model" text="model" >}} that implements an existing Viam [API](/platform/build/program/apis/).
...
When implementing a custom {{< glossary_tooltip term_id="model" text="model" >}} of an existing {{< glossary_tooltip term_id="component" text="component" >}}, valid [APIs](/platform/build/program/apis/) always have the following parameters:
...
When implementing a custom {{< glossary_tooltip term_id="model" text="model" >}} of an existing [service](/platform/build/configure/services/), valid [APIs](/platform/build/program/apis/) always have the following parameters:
...
If you are using unique hardware that does not already have an [appropriate API](/platform/build/program/apis/#component-apis) defined to support it, you can use the [generic API](/platform/build/configure/components/generic/) to add support for that unique hardware type to your machine.

Some use cases may require you to define a new API, or to deploy custom components using a server on a remote part.
For more information, see [Advanced Modular Resources](/platform/registry/advanced/).
...
For more information see [Naming your model](/platform/registry/upload/#naming-your-model-namespacerepo-namename).
...
The following example module registers a modular resource implementing Viam's built-in [Base API](/platform/build/configure/components/base/#api) (`rdk:service:base`) as a new model, `"mybase"`, using the model family `acme:demo:mybase`.
...
The following example module registers a modular resource implementing Viam's built-in [Base API](/platform/build/configure/components/base/#api) (`rdk:service:base`) as a new {{< glossary_tooltip term_id="model" text="model" >}}, `"mybase"`, using the model family `acme:demo:mybase`.
...
To [add a module](/platform/registry/configure/) to the configuration of your robot, you need to have an [executable](https://en.wikipedia.org/wiki/Executable) that:
...
Make sure to [prepare a Python virtual environment](/platform/build/program/python-venv/) in the directory your module is in to ensure your module has access to any required libraries:
...
Make sure to [prepare a Python virtual environment](/platform/build/program/python-venv/) in the directory your module is so your module has access to any required libraries.
Compile your module as follows:
...
Once you have created your module, you can use the [Viam CLI](/platform/fleet/cli/) to [upload your module](/platform/registry/upload/) to the [Viam registry](https://app.viam.com/registry) to share it with other Viam users or just to other users in your organization.
For added convenience, you can configure [automated uploads for new module versions](/platform/registry/upload/#update-an-existing-module-using-a-github-action) through a continuous integration (CI) workflow, using a GitHub Action.

You can also add your module to your robot as a [local module](/platform/registry/configure/#local-modules), without uploading it to the Viam registry.

{{< cards >}}
{{% card link="/platform/registry/upload/" %}}
{{% manualcard link="/platform/registry/configure/#local-modules" %}}
