...
The [Machine Learning (ML) model service](/platform/build/configure/services/ml/) allow you to deploy machine learning models to your smart machine.
Vision services, like [an `"mlmodel"` detector](/platform/build/configure/services/vision/detection/#configure-an-mlmodel-detector) or [classifier](/platform/build/configure/services/vision/classification/#configure-an-mlmodel-classifier), enable your machines to identify and classify objects in images with the deployed models' predictions.

The two services work closely together, with the vision service relying on the deployed ML model to make inferences.
If you are [designing your own ML Model service](/platform/registry/), you must try to make your ML models' shapes match the input and output tensors the `mlmodel` vision service expects to work with if you want the two services to coordinate in classification or detection.

To be able to use a deployed ML model, the `mlmodel` vision service checks for descriptions of these characteristics in the [metadata](/platform/build/configure/services/ml/#metadata) of the model, as defined in [the Python SDK](https://python.viam.dev/autoapi/viam/gen/service/mlmodel/v1/mlmodel_pb2/index.html#viam.gen.service.mlmodel.v1.mlmodel_pb2.Metadata).
For an example of this, see [Example Metadata](#example-metadata).

## Input tensor: `input_info` in metadata

For both [classification](/platform/build/configure/services/vision/classification/) and [detection](/platform/build/configure/services/vision/detection/) models, the vision service sends a single input tensor to the ML Model with the following structure:
...
For [classifications](/platform/build/configure/services/vision/classification/):
...
For [detections](/platform/build/configure/services/vision/detection/):
...
For example, a TF lite detector model that works with the vision service is structured with the following [metadata](/platform/build/configure/services/ml/#metadata):
