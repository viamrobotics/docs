...
Viam provides an example {{< glossary_tooltip term_id="modular-resource" text="modular resource" >}} written in C++ that extends the [ML model](/platform/build/configure/services/ml/) service to run any TensorFlow Lite model.
...
1. Navigate to [the Viam app](https://app.viam.com) in your browser and [add a new robot](/platform/fleet/robots/#add-a-new-robot).
...
This generated configuration features the minimum required configuration to support this tutorial: `services` parameters for the [ML model](/platform/build/configure/services/ml/) service and `modules` parameters for the `example_mlmodelservice_tflite` module.
...
The `example_mlmodelservice_tflite` module, and the `MLModelService` modular resource it provides, extends the existing [ML model](/platform/build/configure/services/ml/) service to run any TensorFlow Lite model.
...
- Write a client similar to `example_audio_classification_client` that [trains its own model](/platform/ml/train-model/) on existing or incoming data, as long as that model fulfils the TFLite model constraints.
  For example, you might add a [movement sensor](/platform/build/configure/components/movement-sensor/) component to your robot that captures sensor readings to the built-in [data management service](/platform/build/configure/services/data/).
