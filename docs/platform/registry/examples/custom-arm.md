...
The {{< glossary_tooltip term_id="rdk" text="RDK" >}} provides a number of built-in arm {{< glossary_tooltip term_id="model" text="models" >}} that implement the API protocol of the [arm](/platform/build/configure/components/arm/) {{< glossary_tooltip term_id="subtype" text="subtype" >}} of component, such as the `ur5e`, `xArm6`, and `xArm7`.
...
See [Arm Configuration](/platform/build/configure/components/arm/#supported-models) for the current list of built-in models the RDK provides.

{{% /alert %}}

If you have a robot arm that is not already supported by the RDK, create a module that provides a customized model for your arm to [program](/platform/build/program/) and control it with the [arm API](/platform/build/configure/components/arm/#api), or use it with [services](/platform/build/configure/services/) like [Motion](/platform/build/configure/services/motion/), just as you would with a built-in model.

See [Modular Resources](/platform/registry/) for more information.

## Get your arm's kinematics file

The way arms move through space is more complicated than Viam's other [components](/platform/build/configure/components/).
...
This provides the necessary information for the [frame system service](/platform/build/configure/services/frame-system/) and built-in [motion service](/platform/build/configure/services/motion/) to work with the arm.
...
- Follow the instructions on [Configure Complex Kinematic Chains](/reference/internals/kinematic-chain-config/) to write a file detailing the geometry of your arm.
  ...
  - Follow the frame system's guide to [Configure a Reference Frame](/platform/build/configure/services/frame-system/frame-config/) when working out the relative [orientations](/reference/internals/orientation-vector/) of the `links` on your arm.
...
To create a custom arm model, code a module in Python with the module support libraries provided by [Viam's SDKs](/platform/build/program/apis/):
...
Follow [this guide](/platform/registry/create/#code-a-new-resource-model) and select **Go** on the code samples to learn how to code a modular arm in Go.
...
This module template registers a modular resource implementing Viam's built-in [Arm API](/platform/build/configure/components/arm/#api) (`rdk:service:arm`) as a new {{< glossary_tooltip term_id="model" text="model" >}}, `"myarm"`:
...
Follow [these configuration instructions](/platform/registry/configure/) to add your custom resource to your robot.
