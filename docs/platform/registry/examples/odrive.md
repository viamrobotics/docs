...
1. Read through the [ODrive documentation](https://docs.odriverobotics.com/v/latest/getting-started.html) to wire, calibrate, and configure your ODrive natively.
   ...
   Note that `iq_msg_rate_ms` in the `odrive_config_file` defaults to `0`, and you must set this to around `100` to use the [motor API's `SetPower` method](/platform/build/configure/components/motor/#setpower).
...
1. See the [ODrive CAN documentation](https://docs.odriverobotics.com/v/latest/can-guide.html) for detailed information on how to set up CAN on your ODrive.
   ...
   If you are using a Raspberry Pi as your [board](/platform/build/configure/components/board/), you must run `sudo ip link set can0 up type can bitrate <baud_rate>` in your terminal to receive CAN messages.
   See [Troubleshooting: CAN Link Issues](https://github.com/viamrobotics/odrive#can-link-issues) for more information.

   Additionally, make sure you have [enabled SPI communication on your Pi](/platform/get-started/installation/prepare/rpi-setup/) to use several common CANHats.

   {{% /alert %}}

1. Make sure your ODrive is connected to your [board](/platform/build/configure/components/board/) as follows, depending on whether you are using a `serial` or `canbus` connection:
...
Check the [**Logs** tab](/platform/build/program/debug/) of your robot in the Viam app to make sure your ODrive motor has connected and no errors are being raised.
...
2. Configure `iq_msg_rate_ms` to a value appropriate for your usage.
   ...
   You must set this to or around `100` to use the [motor API's `SetPower` method](/platform/build/configure/components/motor/#setpower).
