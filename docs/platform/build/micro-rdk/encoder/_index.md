...
For more information and models supported by the {{< glossary_tooltip term_id="rdk" text="RDK" >}}, see [Encoder Component](/platform/build/configure/components/encoder/).
...
The micro-RDK [encoder API](/platform/build/configure/components/encoder/#api) supports only the following supported client SDK API methods, which operate the same ways as those in the full-featured RDK:

- [`ResetPosition()`](/platform/build/configure/components/encoder/#resetposition)
- [`GetPosition()`](/platform/build/configure/components/encoder/#getposition)
- [`GetProperties()`](/platform/build/configure/components/encoder/#getproperties)
