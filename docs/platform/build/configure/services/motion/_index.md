...
icon: "/platform/build/configure/services/icons/motion.svg"
no_list: true
aliases:
  - "/services/motion/"
# SME: Motion team
...
{{< cards >}}
{{< relatedcard link="/platform/build/configure/services/frame-system/" >}}
...
{{< relatedcard link="/platform/build/configure/components/arm/" >}}
{{< relatedcard link="/platform/build/configure/components/gripper/" >}}
{{< /cards >}}
...
Use the name `"builtin"` to access the built-in motion service in your code with methods like [`FromRobot()`](/platform/build/program/apis/#fromrobot) that require a `ResourceName`.
...
The following code example gets the pose of the tip of a [gripper](/platform/build/configure/components/gripper/) named `my_gripper` which is attached to the end of an arm, in the "world" `reference_frame`:
...
Move a [base](/platform/build/configure/components/base/) component to a [`Pose`](https://python.viam.dev/autoapi/viam/proto/common/index.html#viam.proto.common.Pose) in respect to the origin of a {{< glossary_tooltip term_id="slam" text="SLAM" >}} map.
...
Move a [base](/platform/build/configure/components/base/) component to a destination GPS point, represented in geographic notation _(latitude, longitude)_.
Use a [movement sensor](/platform/build/configure/components/movement-sensor/) to check the location of the robot.

{{< alert title="Usage" color="tip" >}}

`MoveOnGlobe()` is intended for use with the [navigation service](/platform/build/configure/services/navigation/), providing autonomous GPS navigation for rover [bases](/platform/build/configure/components/base/).

To use `MoveOnGlobe()`, your movement sensor must be able to measure the GPS location and orientation of the robot.

Make sure the [movement sensor](/platform/build/configure/components/movement-sensor/) you use supports usage of the following methods in its {{< glossary_tooltip term_id="model" text="model's" >}} implementation of the [movement sensor API](/platform/build/configure/components/movement-sensor/#api).
...
Specifying `heading` in a request to `MoveOnGlobe` is not currently recommended if the minimum turning radius of your component is greater than zero, as this combination may cause high latency in the [motion planning algorithms](/platform/build/configure/services/motion/algorithms/).
...
Translation in obstacles is not supported by the [navigation service](/platform/build/configure/services/navigation/).
...
- `movement_sensor_name` [(ResourceName)](https://python.viam.dev/autoapi/viam/gen/common/v1/common_pb2/index.html#viam.gen.common.v1.common_pb2.ResourceName): The `ResourceName` of the [movement sensor](/platform/build/configure/components/movement-sensor/) that you want to use to check the robot's location.
...
- `configuration` [(Optional[MotionConfiguration])](https://python.viam.dev/autoapi/viam/proto/service/motion/index.html#viam.proto.service.motion.MotionConfiguration): The configuration you want to set across this robot for this motion service. This parameter and each of its fields are optional.
  - `obstacle_detectors` [(Iterable[ObstacleDetector])](https://python.viam.dev/autoapi/viam/proto/service/motion/index.html#viam.proto.service.motion.ObstacleDetector): The names of each [vision service](/platform/build/configure/services/vision/) and [camera](/platform/build/configure/components/camera/) resource pair you want to use for transient obstacle avoidance.
...
- `movementSensorName` [(resource.Name)](https://pkg.go.dev/go.viam.com/rdk/resource#Name): The `resource.Name` of the [movement sensor](/platform/build/configure/components/movement-sensor/) that you want to use to check the robot's location.
...
- `motionConfig` [(\*MotionConfiguration)](https://pkg.go.dev/go.viam.com/rdk/services/motion#MotionConfiguration): The configuration you want to set across this robot for this motion service. This parameter and each of its fields are optional.
  - `ObstacleDetectors` [([]ObstacleDetectorName)](https://pkg.go.dev/go.viam.com/rdk/services/motion#ObstacleDetectorName): The names of each [vision service](/platform/build/configure/services/vision/) and [camera](/platform/build/configure/components/camera/) resource pair you want to use for transient obstacle avoidance.
