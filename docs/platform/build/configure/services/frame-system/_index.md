...
weight: 45
no_list: true
icon: "/platform/build/configure/services/icons/frame-system.svg"
...
![Visualization of a wheeled base configured with motors and a mounted camera in the frame system tab of the Viam app UI](/platform/build/configure/services/frame-system/frame_system_wheeled_base.png)

The frame system is the basis for many of Viam's other services, like [Motion](/platform/build/configure/services/motion/) and [Vision](/platform/build/configure/services/vision/).
It stores the required contextual information to use the position and orientation readings returned by some components.

## Used With

{{< cards >}}
{{< relatedcard link="/platform/build/configure/services/arm/">}}
{{< relatedcard link="/platform/build/configure/components/base/">}}
{{< relatedcard link="/platform/build/configure/components/board/">}}
{{< relatedcard link="/platform/build/configure/services/camera/">}}
{{< relatedcard link="/platform/build/configure/services/gantry/">}}
{{< relatedcard link="/platform/build/configure/services/motor/">}}
{{< /cards >}}

## Configuration

To enable the default frame for a given [component](/platform/build/configure/components/) on a robot, navigate to the **Config** tab of the robot's page in [the Viam app](https://app.viam.com) and click **Components**.
...
![add reference frame pane](/platform/build/configure/services/frame-system/frame_card.png)
...
For [base components](/platform/build/configure/components/base/), Viam considers `+X` to be to the right, `+Y` to be forwards, and `+Z` to be up.
...
- [A Reference Frame:](/platform/build/configure/services/frame-system/frame-config/) A component attached to a static surface
- [Nested Reference Frames:](/platform/build/configure/services/frame-system/nested-frame-config/) A component attached to another, dynamic, component
...
![Default frame system configuration grid visualization for a single component, shown in the Frame System Editor](/platform/build/configure/services/frame-system/frame_system_basic.png)
...
Consider a robot configured with a [`jetson` board](/platform/build/configure/components/board/), wired to a [`webcam` camera](/platform/build/configure/components/camera/webcam/) and a [`wheeled` base](/platform/build/configure/components/base/wheeled/) with two [motors](/platform/build/configure/components/motor/) driving its wheels.
...
![Example robot's default frame configuration shown in the Frame System Editor. All components are stuck on top of each other](/platform/build/configure/services/frame-system/demo_base_unedited.png)
...
![Base translated 200mm forwards shown in the Frame System Editor](/platform/build/configure/services/frame-system/demo_base_edited.png)
...
![Camera translated 50 mm overhead shown in the Frame System Editor](/platform/build/configure/services/frame-system/demo_camera_edited_1.png)
...
![Camera oriented .5 degrees OX shown in the Frame System Editor](/platform/build/configure/services/frame-system/demo_camera_edited_2.png)

Now that the frame system is accurately configured with the robot's spatial orientation, [motion service](/platform/build/configure/services/motion/) methods that take in reference frame information can be utilized.

### Display Options

Click and drag on the **Frame System** visualization to view the display from different angles, and pinch to zoom in and out:

{{<gif webm_src="/platform/build/configure/services/frame-system/frame_system_demo.webm" mp4_src="/platform/build/configure/services/frame-system/frame_system_demo.mp4" alt="The frame system visualization zooming in and out around the example robot with a camera, board, and wheeled base.">}}
...
![Default Perspective Camera view shown in the Frame System Editor](/platform/build/configure/services/frame-system/demo_perspective.png)

{{% /tab %}}
{{% tab name="Orthographic Camera" %}}

![Non-default Orthographic Camera view shown in the Frame System Editor](/platform/build/configure/services/frame-system/demo_orthographic.png)
...
![Demo robot with default box bounds added to the wheeled base, shown in the Frame System Editor](/platform/build/configure/services/frame-system/demo_bound_box.png)

You can adjust the **Size** and **Translation** of a **Geometry** to change these bounds.
For example:

![Demo robot with translated box bounds added to the wheeled base, shown in the Frame System Editor](/platform/build/configure/services/frame-system/demo_bound_box_translation.png)

{{< /tab >}}
{{% tab name="Sphere" %}}

![Demo robot with default sphere bounds added to the wheeled base, shown in the Frame System Editor](/platform/build/configure/services/frame-system/demo_bound_sphere.png)

{{% /tab %}}
{{% tab name="Capsule" %}}

![Demo robot with default capsule bounds added to the wheeled base, shown in the Frame System Editor](/platform/build/configure/services/frame-system/demo_bound_capsule.png)
...
![an example of a logged frame system](/platform/build/configure/services/frame-system/frame_sys_log_example.png)

Consider the example of nested reference frame configuration where [two dynamic components are attached](/platform/build/configure/services/frame-system/nested-frame-config/): a robotic arm, `A`, attaches to a gantry, `G`, which in turn is fixed in place at a point on the `World` of a table.

The resulting tree of reference frames looks like:

![reference frame tree](/platform/build/configure/services/frame-system/frame_tree.png)
...
- In our [example of nested dynamic attachment](/platform/build/configure/services/frame-system/nested-frame-config/), the arm can be managed by the frame system without additional transforms because the base of the arm is fixed with respect to the gantry's platform, and the gantry's origin is fixed with respect to the `world` reference frame (centered at `(0, 0, 0)` in the robot's coordinate system).

  However, an arm with an attached [camera](/platform/build/configure/components/camera/) might generate additional information about the poses of other objects with respect to references frames on the robot.

  With the [vision service](/platform/build/configure/services/vision/), the camera might detect objects that do not have a relationship to a `world` reference frame.

  If a [camera](/platform/build/configure/components/camera/) is looking for an apple or an orange, the arm can be commanded to move to the detected fruit's location by providing an additional transform that contains the detected pose with respect to the camera that performed the detection.
...
- You can pass a detected object's frame information to the `supplemental_transforms` parameter in your calls to Viam's motion service's [`GetPose`](/platform/build/configure/services/motion/#getpose) method.
