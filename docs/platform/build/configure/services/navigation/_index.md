...
weight: 60
no_list: true
icon: "/platform/build/configure/services/icons/navigation.svg"
images: ["/platform/build/configure/services/icons/navigation.svg"]
...
The Navigation service is the stateful definition of Viam's [motion service](/platform/build/configure/services/motion/).
It uses GPS to autonomously navigate a rover [base](/platform/build/configure/components/base/) to user defined endpoints called waypoints.
...
{{< cards >}}
{{< relatedcard link="/platform/build/configure/components/base/" required="yes" >}}
{{< relatedcard link="/platform/build/configure/components/movement-sensor/" required="yes" >}}
{{< relatedcard link="/platform/build/configure/components/camera/" >}}
{{< /cards >}}
...
You must configure a [base](/platform/build/configure/components/base/) with [movement sensors](/platform/build/configure/components/movement-sensor/) as part of your robot to configure a Navigation service.

To use the navigation service, configure a stack of movement sensors that implement the following methods in their {{< glossary_tooltip term_id="model" text="models'" >}} implementations of the [movement sensor API](/platform/build/configure/components/movement-sensor/#api):
...
- [`SetVelocity()`](/platform/build/configure/components/base/#setvelocity)
- [`GetGeometries()`](/platform/build/configure/components/base/#getgeometries)
- [`GetProperties()`](/platform/build/configure/components/base/#getproperties)
...
![An example configuration for a Navigation service in the Viam app Config Builder.](/platform/build/configure/services/navigation/navigation-ui-config.png)
...
| `base` | string | **Required** | The `name` you have configured for the [base](/platform/build/configure/components/base/) you are operating with this service. |
| `movement_sensor` | string | **Required** | The `name` of the [movement sensor](/platform/build/configure/components/movement-sensor/) you have configured for the base you are operating with this service. |
| `motion_service` | string | Optional | The `name` of the [motion service](/platform/build/configure/services/motion/) you have configured for the base you are operating with this service. If you have not added a motion service to your robot, the default motion service will be used. Reference this default service in your code with the name `"builtin"`. |
| `obstacle_detectors` | array | Optional | An array containing objects with the `name` of each [`"camera"`](/platform/build/configure/components/camera/) you have configured for the base you are navigating, along with the `name` of the [`"vision_service"`](/platform/build/configure/services/motion/) you are using to detect obstacles. Note that any vision services on remote parts will only be able to access cameras on the same remote part. |
...
| `degs_per_sec` | float | Optional | The default angular velocity for the [base](/platform/build/configure/components/base/) in degrees per second. <br> Default: `60` |
| `meters_per_sec` | float | Optional | The default linear velocity for the [base](/platform/build/configure/components/base/) in meters per second. <br> Default: `0.3` |
| `obstacles` | obj | Optional | Any obstacles you wish to add to the robot's path. See the [motion service](/platform/build/configure/services/motion/) for more information. |

### Configure and calibrate the frame system service for GPS navigation

{{% alert title="Info" color="info" %}}

The [frame system service](/platform/build/configure/services/frame-system/) is an internally managed and mostly static system for storing the reference frame of each component of a robot within a coordinate system configured by the user.

It stores the required contextual information for Viam's services like [Motion](/platform/build/configure/services/motion/) and [Vision](/platform/build/configure/services/vision/) to use the position and orientation readings returned by components like [movement sensors](/platform/build/configure/components/movement-sensor/).
...
To start, add the frame system service to your rover [base](/platform/build/configure/components/base/) and [movement sensor](/platform/build/configure/components/movement-sensor/).

- Navigate to the **Config** tab of your robot's page in [the Viam app](https://app.viam.com).
  Scroll to the card with the name of your base:

  {{< imgproc src="/platform/build/configure/services/navigation/click-add-frame-ui.png" alt="The button to add a frame selected with the cursor on the Viam app config builder." resize="500x" >}}
...
- Click **Add Frame**.
  ...
  - Configure a **Geometry** for the base that reflects its physical dimensions.
    Reference [these instructions](/platform/build/configure/services/frame-system/#bounding-geometries) to configure your geometry and measure the physical dimensions of your base.

    ![The frame card for the base in the Viam app config builder.](/platform/build/configure/services/navigation/full-frame-base-ui.png)
...
- Scroll to the card with the name of your movement sensor.
  ...
  - Type in the `name` of your base to specify this component as the `parent` of the sensor in the reference frame coordinate system, and click **Save Config** to save your configuration.
    See [how to configure nested reference frames](/platform/build/configure/services/frame-system/nested-frame-config/) for an explanation of this configuration process.
    ![The frame card for the base in the Viam app config builder.](/platform/build/configure/services/navigation/full-frame-movement-sensor-ui.png)
  ...
  - Give the movement sensor a **Translation** that reflects where it is mounted on your base, measuring the coordinates with respect to the origin of the base.
    ...
    See [the frame system service](/platform/build/configure/services/frame-system/#configuration) for more information, and [the Viam Internals](/reference/internals/orientation-vector/) for a detailed guide on conducting this measurement.
...
See the [motion service](/platform/build/configure/services/motion/) for more information.
...
![An example control interface for a Navigation service in the Viam app Control Tab.](/platform/build/configure/services/navigation/navigation-control-card.png)

## Navigation Concepts

The following concepts are important to understand when utilizing the navigation service.
Each concept is a type of relative or absolute measurement, taken by a [movement sensor](/platform/build/configure/components/movement-sensor/), which can then be utilized by your robot to navigate across space.

Here's how to make use of the following types of measurements:

- [Compass Heading](/platform/build/configure/services/navigation/#compass-heading)
- [Orientation](/platform/build/configure/services/navigation/#orientation)
- [Angular Velocity](/platform/build/configure/services/navigation/#angular-velocity)
- [Position](/platform/build/configure/services/navigation/#position)
- [Linear Acceleration](/platform/build/configure/services/navigation/#linear-acceleration)
- [Linear Velocity](/platform/build/configure/services/navigation/#linear-velocity)

### Compass Heading

The following {{< glossary_tooltip term_id="model" text="models" >}} of [movement sensor](/platform/build/configure/components/movement-sensor/) take compass heading measurements:

- [gps-nmea](/platform/build/configure/components/movement-sensor/gps/gps-nmea/) - some GPS hardware only report heading while moving.
- [gps-nmea-rtk-pmtk](/platform/build/configure/components/movement-sensor/gps/gps-nmea-rtk-pmtk/) - some GPS hardware only report heading while moving.
- [gps-nmea-rtk-serial](/platform/build/configure/components/movement-sensor/gps/gps-nmea-rtk-serial/) - some GPS hardware only report heading while moving.
- [imu-wit](/platform/build/configure/components/movement-sensor/imu/imu-wit/)
...
To read compass headings, [configure a capable movement sensor](/platform/build/configure/components/movement-sensor/#supported-models) on your robot.
Then use the movement sensor API's [`GetCompassHeading()`](/platform/build/configure/components/movement-sensor/#getcompassheading) method to get readings from the sensor.

### Orientation

The following {{< glossary_tooltip term_id="model" text="models" >}} of [movement sensor](/platform/build/configure/components/movement-sensor/) take orientation measurements:
...
You can choose the origin reference frame by configuring it using Viam's [frame system](/platform/build/configure/services/frame-system/).
The `GetOrientation` readings will report orientations relative to that initial frame.

To read orientation, first [configure a capable movement sensor](/platform/build/configure/components/movement-sensor/#supported-models) on your robot.
Additionally, follow [these instructions](/platform/build/configure/services/frame-system/#configuration) to configure the geometries of each component of your robot within the [frame system](/platform/build/configure/services/frame-system/).
Then use the movement sensor API's [`GetOrientation()`](/platform/build/configure/components/movement-sensor/#getorientation) method to get orientation readings.

### Angular Velocity

The following {{< glossary_tooltip term_id="model" text="models" >}} of the [movement sensor](/platform/build/configure/components/movement-sensor/) component take angular velocity measurements:

- [imu-wit](/platform/build/configure/components/movement-sensor/imu/imu-wit/)
- [wheeled-odometry](/platform/build/configure/components/movement-sensor/wheeled-odometry/)
- [gyro-mpu6050](/platform/build/configure/components/movement-sensor/mpu6050/)
...
To get an angular velocity reading, first [configure a capable movement sensor](/platform/build/configure/components/movement-sensor/#supported-models) on your robot.
Then use the movement sensor API's [`GetAngularVelocity()`](/platform/build/configure/components/movement-sensor/#getangularvelocity) method to get angular velocity readings from the sensor.

### Position

The following {{< glossary_tooltip term_id="model" text="models" >}} of the [movement sensor](/platform/build/configure/components/movement-sensor/) component take position measurements:

- [gps-nmea](/platform/build/configure/components/movement-sensor/gps/gps-nmea/)
- [gps-nmea-rtk-pmtk](/platform/build/configure/components/movement-sensor/gps/gps-nmea-rtk-pmtk/)
- [gps-nmea-rtk-serial](/platform/build/configure/components/movement-sensor/gps/gps-nmea-rtk-serial/)
...
To get a position, [configure a capable movement sensor](/platform/build/configure/components/movement-sensor/#supported-models) on your robot.
Then use the movement sensor API's [`GetPosition()`](/platform/build/configure/components/movement-sensor/#getposition) method to get position readings from the sensor.

### Linear Velocity

The following {{< glossary_tooltip term_id="model" text="models" >}} of [movement sensor](/platform/build/configure/components/movement-sensor/) take linear velocity measurements:

- [gps-nmea](/platform/build/configure/components/movement-sensor/gps/gps-nmea/)
- [gps-nmea-rtk-pmtk](/platform/build/configure/components/movement-sensor/gps/gps-nmea-rtk-pmtk/)
- [gps-nmea-rtk-serial](/platform/build/configure/components/movement-sensor/gps/gps-nmea-rtk-serial/)
- [wheeled-odometry](/platform/build/configure/components/movement-sensor/wheeled-odometry/) (provides a relative estimate only based on where the base component has started)
...
To get linear velocity, [configure a capable movement sensor](/platform/build/configure/components/movement-sensor/#supported-models) on your robot.
Then use the movement sensor API's [`GetLinearVelocity()`](/platform/build/configure/components/movement-sensor/#getlinearvelocity) method to get linear velocity readings from the sensor.

### Linear Acceleration

The following {{< glossary_tooltip term_id="model" text="models" >}} of [movement sensor](/platform/build/configure/components/movement-sensor/) take linear acceleration measurements:

- [accel-adxl345](/platform/build/configure/components/movement-sensor/adxl345/)
- [imu-wit](/platform/build/configure/components/movement-sensor/imu/imu-wit/)
- [gyro-mpu6050](/platform/build/configure/components/movement-sensor/mpu6050/)
...
You can use linear acceleration readings to determine the rate of change of the [linear velocity](/platform/build/configure/services/navigation/#linear-velocity) of your robot, or, the acceleration at which your robot is moving through space.

To get linear acceleration, [configure a capable movement sensor](/platform/build/configure/components/movement-sensor/#supported-models) on your robot.
Then use the movement sensor API's [`GetLinearAcceleration()`](/platform/build/configure/components/movement-sensor/#getlinearacceleration) method to get linear acceleration readings from the sensor.
