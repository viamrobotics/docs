...
{{<gif webm_src="/platform/build/configure/services/vision/mug-classifier.webm" mp4_src="/platform/build/configure/services/vision/mug-classifier.mp4" alt="A classification model run against an image containing a mug." max-width="250px" class="alignright">}}

If you have images stored in the [Viam cloud](/platform/data/), you can run your classifier against your images in the [Viam app](https://app.viam.com/).

1. Navigate to the [Data tab](/platform/data/view/) and click on the **Images** subtab.
...
If you intend to use the classifier with a camera that is part of your robot, you can test your classifier from the [**Control tab**](/platform/fleet/robots/#control) or with code:

1. Configure a [camera component](/platform/build/configure/components/camera/).
2. (Optional) If you would like to see classifications from the **Control tab**, configure a [transform camera](/platform/build/configure/components/camera/transform/) with the following attributes :
...
4. Navigate to the **Control** tab, click on your transform camera and toggle it on.
   ...
   ![Model recognizes a star on camera feed](/platform/build/configure/services/model-on-camera.png)
