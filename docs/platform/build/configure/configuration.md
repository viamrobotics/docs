...
image: "/platform/get-started/installation/thumbnails/configure.svg"
imageAlt: "Configure a Robot"
images: ["/platform/get-started/installation/thumbnails/configure.svg"]
...
You can also store your config file in a custom location if desired. See [Run `viam-server`](/platform/get-started/installation/manage/#run-viam-server) for more information.
{{< /alert >}}
...
If your robot will never connect to the internet, you can also create a [local configuration file](/reference/appendix/local-configuration-file/) on the robot itself.
{{% /alert %}}
...
![The CONFIG tab of the Viam app with a box highlighting the CONFIG tab and the Mode selector (where you can select Builder or Raw JSON).](/platform/build/configure/configuration/config-tab.png)
...
See [Example JSON configuration file](/reference/appendix/local-configuration-file/#example-json-configuration-file) for an additional example.
...
- `attributes`: A struct to define things like how the component is wired to the robot, its dimensions, and other specifications; attributes vary widely between models.
  See the [component documentation](/platform/build/configure/components/) for a given component type and model for more details.
...
- `depends_on`: Any components that a given component relies upon, and that must be initialized on boot before this component is initialized.
  ...
  For example, a [`gpio` motor](/platform/build/configure/components/motor/gpio/) depends on the `board` to which it is wired, but it has a dedicated `board` attribute and `viam-server` will automatically initialize that board before it looks for the motor.
...
For specific information on how to configure each supported component type, see the [components documentation](/platform/build/configure/components/).
...
Meanwhile the **Code sample** tab will also update to include code for some basic interaction with that component using the Viam [SDKs](/platform/build/program/apis/).
...
[Services](/platform/build/configure/services/) are built-in software packages that make it easier to add complex capabilities such as motion planning or object detection to your robot.
...
See the [services documentation](/platform/build/configure/services/) for more information.

## Modules

[Modular resources](/platform/registry/) are a way to add resource types or models that are not built into Viam.

To add a modular resource as a component or service of your robot, configure a module per the [modular resource documentation](/platform/registry/).
...
![The PROCESSES subtab of the CONFIG tab with a process called run-my-code configured. The executable is python3, the argument is my_cool_script.py, and the working directory is /home/myName/project. Logging is turned on and execute once is turned off.](/platform/build/configure/configuration/process-fancy.png)
...
For an example of adding a fragment to a robot, see the [Viam Rover fragment tutorial](/platform/get-started/try-viam/rover-resources/rover-tutorial-fragments/).
...
Find more information in the [frame system documentation](/platform/build/configure/services/frame-system/).

## Troubleshooting

If you run into issues, here are some things to try:

- Check the [**Logs** tab](/platform/fleet/robots/#logs) to view log messages and errors from `viam-server`.
  You can also [access the local log file](/platform/get-started/installation/manage/#view-viam-server-logs) on your robot if needed.
...
- [Fleet Management](/platform/fleet/)
- [Data Management](/platform/data/)
- [Machine Learning](/platform/ml/)

However, if you are configuring a robot that can never connect to the internet, you can create a [local configuration file](/reference/appendix/local-configuration-file/) on your robot.
A locally-configured robot will not be able to access Viam's cloud features.

## Next Steps

After configuring your robot, you can use the [Viam SDKs](/platform/build/program/apis/) to program and control your robot.
