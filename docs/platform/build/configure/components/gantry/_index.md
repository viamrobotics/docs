...
{{<imgproc src="/platform/build/configure/components/gantry/gantry-illustration.png" resize="300x" declaredimensions=true alt="Example of what a multi-axis robot gantry looks like as a black and white illustration of an XX YY mechanical gantry.">}}
</div>
...
- A [board](/platform/build/configure/components/board/) or [controller](/platform/build/configure/components/input-controller/) component that can detect changes in voltage on GPIO pins
- A [motor](/platform/build/configure/components/motor/) that can move linear rails
  - Encoded motor: See [DC motor with encoder](/platform/build/configure/components/motor/gpio/encoded-motor/) and [encoder component](/platform/build/configure/components/encoder/).
  - Stepper motor: See [Stepper motor](/platform/build/configure/components/motor/gpiostepper/).
...
Get all the geometries associated with the gantry in its current configuration, in the [frame](/platform/build/configure/services/frame-system/) of the gantry.
