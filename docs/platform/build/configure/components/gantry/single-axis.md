...
Before configuring the gantry, configure any [motor components](/platform/build/configure/components/motor/) that are part of the gantry.
...
![Creation of a single-axis gantry component in the Viam app config builder.](/platform/build/configure/components/gantry/single-axis-ui-config.png)
...
| `motor` | string | **Required** | `name` of the [motor](/platform/build/configure/components/motor/) that moves the gantry's actuator. |
| `board`  |  string | Optional | `name` of the [board](/platform/build/configure/components/board/) containing the limit switches and pins. If `limit_pins` exist, `board` is required. |
| `limit_pins`  | object | Optional | The `boards`'s pins attached to the limit switches on either end. If the [motor](/platform/build/configure/components/motor/) used does not include an [encoder](/platform/build/configure/components/motor/gpio/encoded-motor/), `limit_pins` are required to be set. |
