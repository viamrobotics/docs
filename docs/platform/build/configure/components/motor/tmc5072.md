...
![Config panel for a TMC5072 motor with attributes filled in according to the Raw JSON tab.](/platform/build/configure/components/motor/tmc5072-config-ui.png)
...
| ---- | ---- | --------- | ----------- |
| `board` | string | **Required** | `name` of the [board](/platform/build/configure/components/board/) to which the motor controller is wired. |
| `spi_bus` | string | **Required** | The board [SPI bus](/platform/build/configure/components/board/#spis) over which the TMC chip communicates with the board. |
