...
Whereas a basic low-level stepper driver supported by the [`gpiostepper` model](/platform/build/configure/components/motor/gpiostepper/) sends power to a stepper motor based on PWM signals from GPIO pins, the DMC40x0 motion controller has many motion control features.
When using it, you do not need to configure a [board component](/platform/build/configure/components/board/) because it handles computation and signal creation on the motion controller itself.
...
![A DMC4000 motor config with the attributes configured per the Raw JSON on the next tab in this doc.](/platform/build/configure/components/motor/dmc4000-config-ui.png)
