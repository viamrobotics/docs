...
To configure a `gpiostepper` motor as a component of your robot, first configure the [board](/platform/build/configure/components/board/) to which the motor driver is wired.
Then, add the motor:
...
![Screenshot of a gpiostepper motor config with the step and dir pins configured to pins 13 and 15.](/platform/build/configure/components/motor/gpiostepper-config-ui.png)
...
![motor-gpiostepper-json.](/platform/build/configure/components/motor/motor-gpiostepper-json.png)
...
| `pins` | object | **Required** |  A struct containing the [board](/platform/build/configure/components/board/) {{< glossary_tooltip term_id="pin-number" text="pin numbers" >}} that the `step` and `dir` pins of the motor driver are wired to. |
