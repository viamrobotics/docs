...
A _board_ component on your robot communicates with the other [components](/platform/build/configure/components/) of the robot.
...
| [`pca9685`](pca9685/) | [PCA9685 Arduino I<sup>2</sup>C Interface](https://www.adafruit.com/product/815), a 16-channel [I<sup>2</sup>C](#i2cs) [PWM](https://docs.arduino.cc/learn/microcontrollers/analog-output)/[servo](/platform/build/configure/components/servo/) driver peripheral |
...
If you are using the micro-RDK, navigate to [Micro-RDK Board](/platform/build/micro-rdk/board/) for supported model information.
...
- `name` [(str)](https://docs.python.org/3/library/stdtypes.html#text-sequence-type-str): Name of the analog reader you want to retrieve. Set as the `"name"` property [in configuration](/platform/build/configure/components/board/#digital_interrupts).
...
- `name` [(str)](https://docs.python.org/3/library/stdtypes.html#text-sequence-type-str): Name of the digital interrupt you want to retrieve. Set as the `"name"` property [in configuration](/platform/build/configure/components/board/#digital_interrupts).
...
- `name` [(string)](https://pkg.go.dev/builtin#string): Name of the digital interrupt you want to retrieve. Set as the `"name"` property [in configuration](/platform/build/configure/components/board/#digital_interrupts).
...
Get all the geometries associated with the board in its current configuration, in the [frame](/platform/build/configure/services/frame-system/) of the board.
The [motion](/platform/build/configure/services/motion/) and [navigation](/platform/build/configure/services/navigation/) services use the relative position of inherent geometries to configured geometries representing obstacles for collision detection and obstacle avoidance while motion planning.
...
With PWM on a _board_, the continuous digital signal output by a GPIO pin is sampled at regular intervals and transmitted to any [hardware components](/platform/build/configure/components/) wired to the pin that read analog signals.
...
The optimal value for PWM Frequency depends on the type and model of [component](/platform/build/configure/components/) you control with the signal output by this pin.
...
The optimal value for PWM Frequency depends on the type and model of [component](/platform/build/configure/components/) you control with the PWM signal output by this pin.
Refer to your device's data sheet for PWM Frequency specifications.
