...
Follow one of our Jetson [setup guides](/platform/get-started/installation/) to prepare your board for running `viam-server` before configuring a `jetson` board.

If you have a CSI camera, follow [these instructions](/platform/registry/examples/csi/) to configure it using the `viam:camera:csi` model.
...
![An example configuration for a jetson board in the Viam app Config Builder.](/platform/build/configure/components/board/jetson-ui-config.png)
