...
The _generic_ component {{< glossary_tooltip term_id="subtype" text="subtype" >}} enables you to add support for unique types of hardware that do not already have an [appropriate API](/platform/build/program/apis/#component-apis) defined for them.

For example, when using an [arm component](/platform/build/configure/components/arm/), it makes sense to use the [arm API](/platform/build/configure/components/arm/#api), which provides specific functionality an arm component needs, such as moving to position or stopping movement.
...
Whenever possible, it is best to use an [existing component API](/platform/build/configure/components/) instead of generic so that you do not have to replicate code.
If you want to use most of an existing API but need just a few other functions, try using the `DoCommand` endpoint and extra parameters to add custom functionality to an [existing subtype](/platform/build/configure/components/), instead of using generic.
...
Get all the geometries associated with the generic component in its current configuration, in the [frame](/platform/build/configure/services/frame-system/) of the generic component.
