...
To configure a `boat` base as a component of your robot, first configure the [board](/platform/build/configure/components/board/) controlling the base and any [motors](/platform/build/configure/components/motor/) attached to the base.
...
| `IMU` | string | **Required** | Name of the [Inertial Measurement Unit](/platform/build/configure/components/movement-sensor/#imu-configuration) in the boat. |
| `Motors` | string[] | **Required** | JSON struct containing the configuration attributes for each motor attached to the boat. |

Each [motor](/platform/build/configure/components/motor/) inside of `Motors` has the following attributes available:
