...
An ultrasonic distance sensor can also be configured as a [sensor](/platform/build/configure/components/sensor/) resource.
When configured as a sensor, you can use the sensor method [`GetReadings()`](/platform/build/configure/components/sensor/#getreadings), rather than the camera method [`GetPointCloud()`](/platform/build/configure/components/camera/#getpointcloud).
{{< /alert >}}
...
![Creation of a ultrasonic camera in the Viam app config builder.](/platform/build/configure/components/camera/configure-ultrasonic.png)
