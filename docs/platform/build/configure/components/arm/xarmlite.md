...
![Web UI configuration panel for an arm of model xArmLite in the Viam app, with Attributes & Depends On dropdowns and the option to add a frame.](/platform/build/configure/components/arm/xArmLite-ui-config.png)
...
See [the frame system service](/platform/build/configure/services/frame-system/) for more information on utilizing and modifying the `"frame"` configuration shown in the `JSON Example` above.
