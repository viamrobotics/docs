...
Acceleration and magnetometer data are available by using the [sensor](/platform/build/configure/components/sensor/) `GetReadings` method, which IMUs wrap.
...
{{< imgproc src="/platform/build/configure/components/movement-sensor/imu-vectornav-builder.png" alt="Creation of an `imu-vectornav` movement sensor in the Viam app config builder." resize="600x" >}}
...
| ---------------------- | ------ | ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `board`                | string | **Required** | The `name` of the [board](/platform/build/configure/components/board/) to which the device is wired.                                                                                                                                                |
| `spi`                  | string |              | The `name` of the [SPI bus](/platform/build/configure/components/board/#spis) over which the device communicates with the board.                                                                                                                    |
