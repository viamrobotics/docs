...
- A [board component](/platform/build/configure/components/board/) that can run a `viam-server` instance.
...
If you are using the micro-RDK, navigate to [Micro-RDK Encoder](/platform/build/micro-rdk/encoder/) for supported model information.
...
Get all the geometries associated with the encoder in its current configuration, in the [frame](/platform/build/configure/services/frame-system/) of the encoder.
