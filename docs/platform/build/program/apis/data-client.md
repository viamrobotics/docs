...
Use the Viam CLI [to generate an api key to authenticate](/platform/fleet/cli/#authenticate).
...
![Part ID displayed in the Viam app.](/platform/build/program/data-client/grab-part-id.png)
...
- `organization_id` ([str](https://docs.python.org/3/library/stdtypes.html#text-sequence-type-str)): ID of organization to delete data from.
  You can obtain your organization id from the [organization settings page](/platform/fleet/organizations/).
...
- `tags` ([Optional[List[str]]](https://docs.python.org/3/library/stdtypes.html#typesseq-list)): Optional list of [image tags](/platform/data/dataset/#image-tags) to allow for tag-based data filtering when retrieving data.
...
Upload tabular data collected on your machine through a specific [component](/platform/build/configure/components/) to the [Viam app](https://app.viam.com).
