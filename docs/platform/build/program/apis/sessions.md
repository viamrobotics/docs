...
For example, imagine a wheeled rover gets a [`SetPower()`](/platform/build/configure/components/base/#setpower) command as the last input from a client before the connection to the robot is interrupted.
...
For more information, see [Client Sessions and Robot Network Connectivity](/platform/build/program/connectivity/).
...
![Heartbeat window configuration screen](/platform/build/program/sessions/heartbeatwindow.png)
...
It is instantiated as part of your [`RobotClient`](/platform/build/program/apis/#robot-api) instance (client of the Robot API).
...
Use your [`RobotClient()`](/platform/build/program/apis/#robot-api) instance to access the [`SessionsClient`](https://pkg.go.dev/go.viam.com/rdk/session) within your client SDK program.
