...
More in-depth information on installing `viam-server` can be found in our [Install Guide](/platform/get-started/installation/#install-viam-server).
{{% /alert %}}
...
For more information, see the [configuration documentation](/platform/build/configure/configuration/#the-config-tab).

{{< alert title="Tip" color="tip" >}}
If you are managing a large fleet, you can use {{< glossary_tooltip term_id="fragment" text="fragments" >}} when [configuring your robot](/platform/build/configure/configuration/).
{{< /alert >}}
...
For local communication between [parts](/platform/build/configure/parts-and-remotes/#robot-parts) Viam uses gRPC or WebRTC.

{{<gif webm_src="/manage/control.webm" mp4_src="/manage/control.mp4" alt="Using the control tab" max-width="800px">}}

<br>

You can also access the control interface using the [Viam mobile app](/platform/fleet/#the-viam-mobile-app), which you can find on the [App Store](https://apps.apple.com/vn/app/viam-robotics/id6451424162) and on [Google Play](https://play.google.com/store/apps/details?id=com.viam.viammobile&hl=en&gl=US).
...
For more information on the SDKs, see [Program your Robot with Viam's SDKs](/platform/build/program/apis/).

There is also a JSON stub you can copy if you wish to have your robot communicate with another robot as a [remote](/platform/build/configure/parts-and-remotes/).
