...
images: ["/platform/ml/train-model.gif"]
webmSrc: "/platform/ml/train-model.webm"
mp4Src: "/platform/ml/train-model.mp4"
videoAlt: "Add a bounding box around the dog in an image."
aliases:
  - /platform/data/train-model/
...
You can add classification tags or bounding boxes to [images collected](/platform/build/configure/services/data/configure-data-capture/) by robots, add them to a dataset, and use the annotated data to train a **Single Label Classification Model**, **Multi Label Classification Model** or **Object Detection Model** within Viam.
...
After [creating a dataset](/platform/data/dataset/), navigate to the **DATA** tab and the **DATASETS** subtab.
...
![The trained model](/platform/ml/stars-model.png)

{{< alert title="Note" color="note" >}}

Your [dataset](/platform/data/dataset/) is not versioned.
...
If you [deploy a model](/platform/build/configure/services/ml/) to a robot, Viam automatically assumes that this is the `latest` version of the model and that you would always like to deploy the `latest` version of the model to the robot.
...
If you do not want Viam to automatically deploy the `latest` version of the model, you can change `packages` configuration in the [Raw JSON robot configuration](/platform/build/configure/configuration/#the-config-tab).
...
{{< cards >}}
{{% manualcard link="/platform/build/configure/services/ml/" %}}
