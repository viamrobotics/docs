...
- The Pi is connected to [the Viam app](https://app.viam.com).
  To add your Pi to the Viam app, refer to [the rover setup guide](/platform/get-started/try-viam/rover-resources/rover-tutorial/#connect-to-the-viam-app).
...
{{<imgproc src="platform/get-started/try-viam/rover-resources/fragments/fragments_tab.png" resize="1200x" alt="Fragments tab inside the Viam app">}}
...
{{<imgproc src="platform/get-started/try-viam/rover-resources/fragments/fragments_list.png" resize="1200x" alt="List of available fragments">}}

Click **Save Config** to save the new configuration.

{{<imgproc src="platform/get-started/try-viam/rover-resources/fragments/fragment_configuration.png" resize="1200x" alt="Fragment configuration">}}

The fragment adds the following components to your robot's JSON configuration:

- A [board component](/platform/build/configure/components/board/pi/) named `local` representing the Raspberry Pi
  - An I<sup>2</sup>C bus for connection to the accelerometer.
- Two [motors](/platform/build/configure/components/motor/gpio/) (`right` and `left`)
  - The configured pin numbers correspond to where the motor drivers are connected to the board.
- Two [encoders](/platform/build/configure/components/encoder/single/), one for each motor
- A wheeled [base](/platform/build/configure/components/base/), an abstraction that coordinates the movement of the right and left motors
  - Width between the wheel centers: 260 mm
  - Wheel circumference: 217 mm
  - Spin slip factor: 1
- A webcam [camera](/platform/build/configure/components/camera/webcam/)
- An [accelerometer](/platform/build/configure/components/movement-sensor/adxl345/)
...
4. Toggle to [**Raw JSON** mode](/platform/get-started/try-viam/try-viam-tutorial/#raw-json).
