...
![The CONFIG tab of the Viam app with a box highlighting the CONFIG tab and the Mode selector (where you can select Builder or Raw JSON).](/build/configure/configuration/config-tab.png)
...
![The PROCESSES subtab of the CONFIG tab with a process called run-my-code configured. The executable is python3, the argument is my_cool_script.py, and the working directory is /home/myName/project. Logging is turned on and execute once is turned off.](/build/configure/configuration/process-fancy.png)
...
Find more information in the [frame system documentation](/build/configure/services/frame-system/).

## Troubleshooting

If you run into issues, here are some things to try:

- Check the [**Logs** tab](/fleet/robots/#logs) to view log messages and errors from `viam-server`.
