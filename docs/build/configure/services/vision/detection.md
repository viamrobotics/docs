...
images: ["/build/configure/services/vision/dog-detector.png"]
aliases:
...
<div class="td-max-width-on-larger-screens">
  <div class="alignright" >
    {{< imgproc alt="A white dog with a bounding box around it labeled 'Dog: 0.71'" src="/build/configure/services/vision/dog-detector.png" resize="300x" declaredimensions=true >}}
  </div>
</div>
...
- [**Object detection (`mlmodel`)**](#configure-an-mlmodel-detector): A detector that draws bounding boxes according to an [ML model](/build/configure/services/ml/) available on the robot’s hard drive.
...
![Color detector panel with color and hue tolerance selection and a field for the segment size](/build/configure/services/vision/color-detector-panel.png)
...
You can [add an existing model](/ml/upload-model/) or [train your own models](/ml/train-model/) for object detection and classification using data from the [data management service](/build/configure/services/data/).
...
To make use of ML models with your machine, use the built-in [ML model service](/build/configure/services/ml/) to deploy and run the model.
...
To create a `mlmodel` classifier, you need an [ML model service with a suitable model](/build/configure/services/ml/).
...
If you intend to use the detector with a camera that is part of your robot, you can test your detector from the [**Control tab**](/fleet/robots/#control) or with code:
...
4. Navigate to the **Control** tab, click on your transform camera and toggle it on.
   ...
   ![Viam app control tab interface showing bounding boxes around two office chairs, both labeled "chair" with confidence score "0.50."](/build/configure/services/vision/chair-detector.png)
