...
The Navigation service is the stateful definition of Viam's [motion service](/build/configure/services/motion/).
...
![An example configuration for a Navigation service in the Viam app Config Builder.](/build/configure/services/navigation/navigation-ui-config.png)
...
| `motion_service` | string | Optional | The `name` of the [motion service](/build/configure/services/motion/) you have configured for the base you are operating with this service. If you have not added a motion service to your robot, the default motion service will be used. Reference this default service in your code with the name `"builtin"`. |
| `obstacle_detectors` | array | Optional | An array containing objects with the `name` of each [`"camera"`](/build/configure/components/camera/) you have configured for the base you are navigating, along with the `name` of the [`"vision_service"`](/build/configure/services/motion/) you are using to detect obstacles. Note that any vision services on remote parts will only be able to access cameras on the same remote part. |
...
| `obstacles` | obj | Optional | Any obstacles you wish to add to the robot's path. See the [motion service](/build/configure/services/motion/) for more information. |

### Configure and calibrate the frame system service for GPS navigation

{{% alert title="Info" color="info" %}}

The [frame system service](/build/configure/services/frame-system/) is an internally managed and mostly static system for storing the reference frame of each component of a robot within a coordinate system configured by the user.

It stores the required contextual information for Viam's services like [Motion](/build/configure/services/motion/) and [Vision](/build/configure/services/vision/) to use the position and orientation readings returned by components like [movement sensors](/build/configure/components/movement-sensor/).
...
- Navigate to the **Config** tab of your robot's page in [the Viam app](https://app.viam.com).
  ...
  {{< imgproc src="/build/configure/services/navigation/click-add-frame-ui.png" alt="The button to add a frame selected with the cursor on the Viam app config builder." resize="500x" >}}
...
- Click **Add Frame**.
  ...
  - Configure a **Geometry** for the base that reflects its physical dimensions.
    Reference [these instructions](/build/configure/services/frame-system/#bounding-geometries) to configure your geometry and measure the physical dimensions of your base.

    ![The frame card for the base in the Viam app config builder.](/build/configure/services/navigation/full-frame-base-ui.png)
...
- Scroll to the card with the name of your movement sensor.
  ...
  - Type in the `name` of your base to specify this component as the `parent` of the sensor in the reference frame coordinate system, and click **Save Config** to save your configuration.
    See [how to configure nested reference frames](/build/configure/services/frame-system/nested-frame-config/) for an explanation of this configuration process.
    ![The frame card for the base in the Viam app config builder.](/build/configure/services/navigation/full-frame-movement-sensor-ui.png)
  ...
  - Give the movement sensor a **Translation** that reflects where it is mounted on your base, measuring the coordinates with respect to the origin of the base.
    ...
    See [the frame system service](/build/configure/services/frame-system/#configuration) for more information, and [the Viam Internals](/reference/internals/orientation-vector/) for a detailed guide on conducting this measurement.
...
See the [motion service](/build/configure/services/motion/) for more information.
...
![An example control interface for a Navigation service in the Viam app Control Tab.](/build/configure/services/navigation/navigation-control-card.png)
...
- [Compass Heading](/build/configure/services/navigation/#compass-heading)
- [Orientation](/build/configure/services/navigation/#orientation)
- [Angular Velocity](/build/configure/services/navigation/#angular-velocity)
- [Position](/build/configure/services/navigation/#position)
- [Linear Acceleration](/build/configure/services/navigation/#linear-acceleration)
- [Linear Velocity](/build/configure/services/navigation/#linear-velocity)
...
You can choose the origin reference frame by configuring it using Viam's [frame system](/build/configure/services/frame-system/).
...
Additionally, follow [these instructions](/build/configure/services/frame-system/#configuration) to configure the geometries of each component of your robot within the [frame system](/build/configure/services/frame-system/).
...
You can use linear acceleration readings to determine the rate of change of the [linear velocity](/build/configure/services/navigation/#linear-velocity) of your robot, or, the acceleration at which your robot is moving through space.
