...
![Visualization of a wheeled base configured with motors and a mounted camera in the frame system tab of the Viam app UI](/build/configure/services/frame-system/frame_system_wheeled_base.png)

The frame system is the basis for many of Viam's other services, like [Motion](/build/configure/services/motion/) and [Vision](/build/configure/services/vision/).
...
![add reference frame pane](/build/configure/services/frame-system/frame_card.png)
...
- [A Reference Frame:](/build/configure/services/frame-system/frame-config/) A component attached to a static surface
- [Nested Reference Frames:](/build/configure/services/frame-system/nested-frame-config/) A component attached to another, dynamic, component
...
![Default frame system configuration grid visualization for a single component, shown in the Frame System Editor](/build/configure/services/frame-system/frame_system_basic.png)
...
![Example robot's default frame configuration shown in the Frame System Editor. All components are stuck on top of each other](/build/configure/services/frame-system/demo_base_unedited.png)
...
![Base translated 200mm forwards shown in the Frame System Editor](/build/configure/services/frame-system/demo_base_edited.png)
...
![Camera translated 50 mm overhead shown in the Frame System Editor](/build/configure/services/frame-system/demo_camera_edited_1.png)
...
![Camera oriented .5 degrees OX shown in the Frame System Editor](/build/configure/services/frame-system/demo_camera_edited_2.png)

Now that the frame system is accurately configured with the robot's spatial orientation, [motion service](/build/configure/services/motion/) methods that take in reference frame information can be utilized.

### Display Options

Click and drag on the **Frame System** visualization to view the display from different angles, and pinch to zoom in and out:

{{<gif webm_src="/build/configure/services/frame-system/frame_system_demo.webm" mp4_src="/build/configure/services/frame-system/frame_system_demo.mp4" alt="The frame system visualization zooming in and out around the example robot with a camera, board, and wheeled base.">}}
...
![Default Perspective Camera view shown in the Frame System Editor](/build/configure/services/frame-system/demo_perspective.png)

{{% /tab %}}
{{% tab name="Orthographic Camera" %}}

![Non-default Orthographic Camera view shown in the Frame System Editor](/build/configure/services/frame-system/demo_orthographic.png)
...
![Demo robot with default box bounds added to the wheeled base, shown in the Frame System Editor](/build/configure/services/frame-system/demo_bound_box.png)

You can adjust the **Size** and **Translation** of a **Geometry** to change these bounds.
For example:

![Demo robot with translated box bounds added to the wheeled base, shown in the Frame System Editor](/build/configure/services/frame-system/demo_bound_box_translation.png)

{{< /tab >}}
{{% tab name="Sphere" %}}

![Demo robot with default sphere bounds added to the wheeled base, shown in the Frame System Editor](/build/configure/services/frame-system/demo_bound_sphere.png)

{{% /tab %}}
{{% tab name="Capsule" %}}

![Demo robot with default capsule bounds added to the wheeled base, shown in the Frame System Editor](/build/configure/services/frame-system/demo_bound_capsule.png)
...
![an example of a logged frame system](/build/configure/services/frame-system/frame_sys_log_example.png)

Consider the example of nested reference frame configuration where [two dynamic components are attached](/build/configure/services/frame-system/nested-frame-config/): a robotic arm, `A`, attaches to a gantry, `G`, which in turn is fixed in place at a point on the `World` of a table.

The resulting tree of reference frames looks like:

![reference frame tree](/build/configure/services/frame-system/frame_tree.png)
...
- In our [example of nested dynamic attachment](/build/configure/services/frame-system/nested-frame-config/), the arm can be managed by the frame system without additional transforms because the base of the arm is fixed with respect to the gantry's platform, and the gantry's origin is fixed with respect to the `world` reference frame (centered at `(0, 0, 0)` in the robot's coordinate system).
  ...
  With the [vision service](/build/configure/services/vision/), the camera might detect objects that do not have a relationship to a `world` reference frame.
...
- You can pass a detected object's frame information to the `supplemental_transforms` parameter in your calls to Viam's motion service's [`GetPose`](/build/configure/services/motion/#getpose) method.
