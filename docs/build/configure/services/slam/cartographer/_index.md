...
1. Configure your `cartographer` SLAM service
   ...
   - configures the `viam:slam:cartographer` service and the [data management service](/build/configure/services/data/)
   ...
   [Data Capture](/build/configure/services/data/configure-data-capture/) continuously monitors and captures your robot's sensor data while the robot is running.
   To avoid incurring charges while not in use, [turn off data capture for your sensors](/build/configure/services/data/configure-data-capture/).
   {{< /alert >}}
...
1. Start a mapping session
   ...
   ![slam RC card start session](/build/configure/services/slam/slam-RC-card-start-session.png)

   While the slam session is starting, you will see a loading screen.

   ![slam RC card wait for session to finish starting](/build/configure/services/slam/slam-RC-card-wait-for-session-to-finish-starting.png)

   Once the slam session has finished starting, your first pointcloud will appear.

   ![slam RC card first pointcloud](/build/configure/services/slam/slam-RC-card-first-pointcloud.png)

   You can see that your cloud slam session is in progress from your **Location** page's **SLAM library** tab.

   ![offline mapping maps computing table](/build/configure/services/slam/offline-mapping-maps-computing-table.png)
   ...
   ![offline mapping available maps](/build/configure/services/slam/offline-mapping-available-maps.png)

   You can click `View map` to view the map in a dynamic pointcloud viewer.

   ![slam library view map](/build/configure/services/slam/slam-library-view-map.png)
...
2.  Start a mapping session
    ...
    ![slam RC card localize only](/build/configure/services/slam/slam-RC-card-localize-only.png)
...
{{<imgproc src="/build/configure/services/slam/offline-mapping-pointcloud-data.png" resize="1200x" declaredimensions=true alt="UI showing captured point clouds">}}

Example of previously captured IMU data:

{{<imgproc src="/build/configure/services/slam/offline-mapping-imu-data.png" resize="1200x" declaredimensions=true alt="UI showing captured sensor data">}}
...
{{<imgproc src="/build/configure/services/slam/offline-mapping-generate-map.png" resize="1200x" declaredimensions=true alt="UI for creating a new map from captured data">}}
