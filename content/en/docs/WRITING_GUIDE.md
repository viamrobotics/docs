---
title: "Viam's Documentation Writing Guide"
linkTitle: "Viam's Documentation Writing Guide"
weight: 95
draft: true
type: "docs"
---
...
- **Cookbook**: Recipes in the cookbook are written with some assumption of familiarity with Viam and its ecosystem. Each is a highly structured document that walks through some common implementation details that a Viam dev might encounter.
...
- **Unless the context assumes a string template or build system, only write code that works in any environment by the software (e.g. Viam, Viamx, etc).**
- **Show, don't tell.** For example, "To use Viam on a page, you can add this to your HTML" (then show the script tag), instead of "To use Viam on a page, you can add a script element with a src attribute, the value of which should be a link to Viam's compiled source".
...
- **Always prefer simpler, plainer language over complex or jargony language.** For example:
  - "you can use Viam with a script element" instead of "in order to initiate the usage of Viam, one possible option is to actually inject it via a script HTML element"
...
We have some dedicated styles to denote something that's worth highlighting in a particular way. These are captured [on this page](https://vitepress.Viamjs.org/guide/markdown.html#custom-containers). **They are to be used sparingly.**
...
This Writing Guide is adapted from the [Vue Docs Writing Guide](https://github.com/vuejs/v2.vuejs.org/blob/master/writing-guide.md)