...
test-code-snippets: test-python-snippets test-go-snippets
...
test-go-snippets:
	...
	for file in static/include/examples/**/*.go; do \
		if [ -f "$$file" ]; then \
			...
			if (cd "$$file_dir" && VIAM_API_KEY="$$VIAM_API_KEY" VIAM_API_KEY_ID="$$VIAM_API_KEY_ID" TEST_ORG_ID="$$TEST_ORG_ID" go run "$$(basename "$$file")"); then \
				passed_files=$$((passed_files + 1)); \
				echo "✅ PASSED: $$file"; \
			else \
				failed_files=$$((failed_files + 1)); \
