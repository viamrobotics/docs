...
- **`/sitetree.json`**: the full site IA as JSON — every section/page with its title, resolved path, and children, mirroring `themes/docsy/layouts/partials/sidebar-tree.html`'s own child-selection and `manualLink` logic so it matches the real sidebar rather than an independent notion of structure. Generated by `layouts/_default/list.sitetree.json` via the `SITETREE` output format on `home`.
- **`/llms.txt`** and **`/llms-full.txt`**: generated by `layouts/_default/list.llms.txt` and `list.llms-full.txt` (the `LLMSTXT`/`LLMSFULLTXT` output formats on `home`) from the curated page list in `data/llms_pages.yaml`--see that file's header comment for the curation strategy (orientation pages get `full: true` and are inlined into llms-full.txt via `render-page-markdown.html`; reference/lookup pages stay link-only, since fetching their own `.md` mirror on demand beats a monolithic dump). `list.llms-full.txt` logs a `warnf` if combined output exceeds roughly 200KB, as a visibility guardrail rather than an enforced cap.
