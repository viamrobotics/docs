...
- **The collision rule**: a qualified output-format template (`list.<format>.md`, `single.<format>.md`) silently shadows its unqualified sibling (`list.md`, `single.md`) site-wide if and only if `<format>`'s name sorts alphabetically *before* `md`--Hugo's template lookup picks the lexically-first candidate, not the more specific one, and there's no build error either way. `markdown` sorts after `md`, so `list.markdown.md` is safe; confirmed empirically by temporarily reintroducing a bare `list.md` alongside a throwaway format sorting before `md`, which shadowed it across every section/home mirror. `single.md` is left unqualified on purpose, even though it's the same class of template: nothing currently forces it to change, and a naive rename isn't free--`layouts/docs/glossary.html`'s type-specific override, `glossary.md`, is *also* unqualified, and a bare `single.md` → `single.<format>.md` rename would outrank it, silently collapsing `/reference/glossary/`'s Markdown mirror from the full set of terms to a handful of lines. If `single.md` ever needs qualifying, rename `glossary.md` to match in the same change.
