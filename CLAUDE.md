...
- **Content negotiation** (`netlify/edge-functions/markdown-negotiation.ts`) serves this same mirror at the canonical (non-`.md`) URL when a client's `Accept` header explicitly prefers it—only a literal `text/markdown` token counts (not `text/*` or `*/*`, which is what ordinary browsers and curl send), and an equal-weight tie against `text/html` goes to markdown. Path computation reuses the mirror's own convention (trim trailing slash, append `.md`), with `/` special-cased to `/index.md` since home has no deeper directory to flatten. `Vary: Accept` is set on every response past the exclusion checks—both the negotiated and the pass-through HTML branch—because Netlify's URL-keyed edge cache would otherwise serve whichever variant it cached first to every later request regardless of that request's own `Accept` header. If the rewrite target isn't `ok` (a `/tags/*` miss, a frontmatter `outputs` override, any future mirror gap), the function falls back to serving normal HTML instead of exposing a 404 at a URL that's supposed to serve HTML.
...
`netlify dev` has two non-obvious failure modes worth knowing before debugging further:

- **Never test edge functions from inside a git worktree.** Netlify CLI's edge-function bundler resolves the wrong project directory when run from a worktree nested under the main checkout (for example, `.claude/worktrees/<name>/`) — it silently bundles edge functions from the *outer* checkout instead, with zero error output. Confirmed by inspecting the generated `.netlify/edge-functions-serve/dev.js`: its `import()` calls pointed at the outer checkout's file paths regardless of which worktree was actually running `netlify dev`. Likely cause: a worktree's `.git` is a gitlink *file*, not a directory, and something in the CLI's project-root resolution walks past it looking for a "real" `.git` directory, landing on the main checkout. Symptom is a function that never fires and never errors—it just isn't there. Test edge functions by running `netlify dev` directly in the main checkout, not a worktree.
- **Local `netlify dev` doesn't apply the Markdown mirror's 200-status `_redirects` rewrites** (the section/home flat-`.md`-to-`index.md` rules generated by `layouts/docs/index.redir`)—these 404 locally while working correctly in production. Plain 301 redirects (from `netlify.toml` or `_redirects`) work fine locally; it's specifically these 200-status, extension-changing rewrites that don't. If a local `.md` URL 404s unexpectedly, check the same URL on production (`docs.viam.com`) before concluding there's a real bug—it may just be this local-dev gap.

Plain `netlify dev` (no `--dir`, no `[dev].framework` override) is the right invocation for exercising edge functions—forcing `--dir <path>` or `framework = "#static"` puts the CLI into "simple static server" mode, which skips edge functions entirely with no warning. If it interactively prompts "Multiple possible dev commands found" (Hugo versus Zola), that's normal for this repo; choose Hugo.
