...
- **`/llms.txt`** and **`/llms-full.txt`**: generated by `layouts/_default/list.llms.txt` and `list.llms-full.txt` (the `LLMSTXT`/`LLMSFULLTXT` output formats on `home`) from the curated page list in `data/llms_pages.yaml` — see that file's header comment for the curation strategy.
