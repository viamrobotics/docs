...
- **`/sitetree.json`**: the full site IA as JSON — every section/page with its title, resolved path, and children, mirroring the core of `layouts/partials/sidebar-tree.html`'s child-selection and `manualLink` logic (this repo's own override of the docsy theme's version — that's the file actually in force, not the vendored copy under `themes/docsy/`) so it broadly matches the real sidebar rather than an independent notion of structure. It's a deliberate simplification, not a byte-for-byte mirror: it hard-excludes `toc_hide` pages (for example, individual tutorials) from the tree entirely, where the real sidebar's handling is more nuanced (conditional display, plus `hide_children`, the default 50-item truncation, and active-path-dependent visibility that have no meaningful analog in a static JSON tree). A section's `path` is its `manualLink` target, which can be identical to one of its own children's `path` (faithful to the sidebar, since a manualLink section and its "Overview" child often point at the same URL)--don't key on `path` as a unique node identifier. Generated by `layouts/_default/list.sitetree.json` through the `SITETREE` output format on `home`.
- **`/llms.txt`** and **`/llms-full.txt`**: generated by `layouts/_default/list.llmstxt.txt` and `list.llmsfulltxt.md` (the `LLMSTXT`/`LLMSFULLTXT` output formats on `home`) from the curated page list in `data/llms_pages.yaml`--see that file's header comment for the curation strategy (orientation pages get `full: true` and are inlined into llms-full.txt through `render-page-markdown.html`; reference/lookup pages stay link-only, since fetching their own `.md` mirror on demand beats a monolithic dump). `list.llmsfulltxt.md` logs a `warnf` if combined output exceeds roughly 200KB, as a visibility guardrail rather than an enforced cap, and `warnf`s if a `data/llms_pages.yaml` entry's `path` doesn't resolve to a real page (both templates guard this the same way). Template filenames must match the output format's `Name` field exactly (case-insensitive)--`baseName` controls the public URL separately, but Hugo's own template lookup keys off the format name, not the desired URL; `list.llms.txt`/`list.llms-full.txt` looked reasonable but silently collided (both rendered the same content) until renamed to match `LLMSTXT`/`LLMSFULLTXT`.
- **`LLMSFULLTXT`'s `mediaType` is `text/markdown`, not `text/plain`**, even though its public URL is `/llms-full.txt`. Shortcode template resolution (unlike the page-template lookup above) keys off the output format's file suffix, not its Name--with `text/plain`'s `.txt` suffix, shortcodes like `alert`/`expand` fell back to their raw HTML variants (no `.txt` counterparts exist), leaking a raw `<div class="alert">`, HTML entities, and even an inline `<svg>` into what's supposed to be a clean plain-text file. `text/markdown` resolves to the `.md` suffix, matching the same `.md` shortcode variants (`alert.md`, `expand.md`, etc.) every page's own Markdown mirror already uses. This makes Hugo emit `/llms-full.md`; `layouts/docs/index.redir` adds `/llms-full.txt /llms-full.md 200` so the public URL stays `.txt`, the same rewrite trick already used for section/home Markdown-mirror URLs (see "Markdown mirror" above). Not fixable by having a `.txt`-suffixed format call the `.md` template as a partial: shortcode resolution is scoped to the whole (page, output format) render pass Hugo is currently executing, fixed once at the top--not to which file does the rendering partway through. Only changing the format's actual `mediaType` flips it. Known tradeoff: `/llms-full.txt` won't resolve in a local `hugo server`/`make build-prod` preview, only on a real Netlify deploy--same blind spot the section/home Markdown-mirror rewrite already has (see `htmltest`'s `CheckLinks: false` above).
- **HTML-escaping risk lives in *printing partials*, not top-level page templates**: a partial whose own file suffix is `.html` or `.txt` HTML-escapes anything it prints through naked top-level output (`{{ .Title }}`, etc.), regardless of the calling template's own kind, suffix, or `isPlainText`/`isHTML` settings. Bare interpolation directly inside a `list.<format>`/`single.<format>` page template never escapes--confirmed by direct testing across `list.llmstxt.txt`, `list.llmsfulltxt.md`, `list.markdown.md`, and `single.md`, all with no `safeHTML` needed. `render-page-markdown.html` sidesteps the real risk by composing a string and using `{{ return }}` instead of printing, matching `render-inner-md.html`'s existing pattern--any future partial called from a plain-text output format should do the same rather than printing.
- **`list.markdown.md`, not `list.md`**: two `list.*` templates sharing a file suffix can collide even when their names differ--Hugo's `list.<format>` lookup isn't a strict "must match this format's Name" rule; it's ambiguous whenever multiple `list.*.<suffix>` candidates share that suffix. Adding `list.llmsfulltxt.md` (suffix `.md`, since `LLMSFULLTXT`'s `mediaType` is `text/markdown`) caused Hugo to render it in place of the unqualified `list.md` for every section's and home's own real Markdown mirror--a silent, site-wide regression this system's own build-and-diff verification didn't catch, found only by an independent review. Fixed by renaming `list.md` to `list.markdown.md`, qualifying it the same way every other output format's template already is. Any future output format sharing an already-used suffix needs its `list.*`/`single.*` template name-qualified from the start, not just the newest one added.
